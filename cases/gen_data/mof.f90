...
!=================
  ...
  Subroutine NormParkerYoungs(f, norm)
    ...
    mm1 = f(1,1,1)+f(1,1,3)+f(1,3,1) &
        +f(1,3,3)+2.0_8*(f(1,1,2)+f(1,3,2)&
        +f(1,2,1)+f(1,2,3))+4.0_8*f(1,2,2)
    mm2 = f(3,1,1)+f(3,1,3)+f(3,3,1) &
        +f(3,3,3)+2.0_8*(f(3,1,2)+f(3,3,2) &
        +f(3,2,1)+f(3,2,3))+4.0_8*f(3,2,2)
    norm(1) = mm1 - mm2

    mm1 = f(1,1,1)+f(1,1,3)+f(3,1,1) &
        +f(3,1,3)+2.0_8*(f(1,1,2)+f(3,1,2) &
        +f(2,1,1)+f(2,1,3))+4.0_8*f(2,1,2)
    mm2 = f(1,3,1)+f(1,3,3)+f(3,3,1) &
        +f(3,3,3)+2.0_8*(f(1,3,2)+f(3,3,2) &
        +f(2,3,1)+f(2,3,3))+4.0_8*f(2,3,2)
    norm(2) = mm1 - mm2

    mm1 = f(1,1,1)+f(1,3,1)+f(3,1,1) &
        +f(3,3,1)+2.0_8*(f(1,2,1)+f(3,2,1) &
        +f(2,1,1)+f(2,3,1))+4.0_8*f(2,2,1)
    mm2 = f(1,1,3)+f(1,3,3)+f(3,1,3) &
        +f(3,3,3)+2.0_8*(f(1,2,3)+f(3,2,3) &
        +f(2,1,3)+f(2,3,3))+4.0_8*f(2,2,3)
    norm(3) = mm1 - mm2
  ...
  Real(8) Function FloodSZ_Backward(nr,cc)
    ...
    FloodSZ_backward = alpha + DMIN1(0.0_8,nr(1)) + DMIN1(0.0_8,nr(2)) + &
        &                 DMIN1(0.0_8,nr(3))
    return
  ...
  Subroutine MOFZY(f, c, norm, Init_Norm)
    ...
    Real(8), Parameter :: local_tol = 1.0e-8
    Real(8), Parameter :: CenTol = 1e-13
    ! Real(sp), Parameter :: GaussNewtonTol = 1e-13
    Real(8), Parameter :: MOF_Pi = 3.1415926535897932d0
    Real(8), Parameter :: epsc = 1.0e-12
    Real(8), Intent(In)  :: f
    Real(8), Intent(In)  :: c(3)
    Real(8), Intent(Out) :: norm(3)
    Real(8), Optional, Intent(In) :: Init_Norm(3)

    Real(8), Dimension(3)   :: norm_2
    Real(8) :: delta_theta
    Real(8) :: delta_theta_max
    Real(8), Dimension(2)   :: delangle, angle_init, angle_previous, new_angle
    Real(8), Dimension(2)   :: angle_base, angle_plus, angle_minus
    Real(8), Dimension(2)   :: err_plus, err_minus
    Real(8), Dimension(3)   :: dbase, dopt, dp, dm
    Real(8), Dimension(3,2) :: d_plus, d_minus
    Real(8), Dimension(3)   :: cenopt, cenp, cenm, cen_init
    Real(8), Dimension(3,2) :: cen_plus, cen_minus
    Real(8), Dimension(3,2) :: dgrad
    Real(8), Dimension(2,2) :: JTJ, JTJINV
    Real(8), Dimension(2)   :: RHS
    Real(8) :: DET
    Real(8), Dimension(3,MOFITERMAX+1) :: d_array, cen_array
    Real(8), Dimension(2,MOFITERMAX+1) :: angle_array
    Real(8), Dimension(MOFITERMAX+1)   :: err_array

    Real(8) :: err, err_local_min
    Real(8) :: scale
    ...
    Else
      scale = 1.0_8 / sqrt(c(1)**2.0_8 + c(2)**2.0_8 +c(3)**2.0_8)
      If (scale> epsc) Then
        Norm_2(1) = - c(1) * scale
        Norm_2(2) = - c(2) * scale
        Norm_2(3) = - c(3) * scale
    ...
    enddo
    delta_theta = MOF_Pi / 180.0_8  ! 1 degree=pi/180
    delta_theta_max = 10.0_8 * MOF_Pi / 180.0_8  ! 10 degrees
    ...
    Do While ((iter.lt.MOFITERMAX).and. &
      ...
      Do i_angle=1,2
        ...
        ! jacobian matrix:
        do dir=1,3
          dgrad(dir,i_angle)=(dp(dir)-dm(dir))/(2.0_8*delta_theta)
        enddo
  ...
  Subroutine FindCentroid(angle, vof, cen_mof)
    Implicit None
    Real(8), Intent(In) :: angle(2)
    Real(8), Intent(In) :: vof
    Real(8), Intent(Out) :: cen_mof(3)
    Real(8) :: cen_sz(3)
    Real(8) :: norm(3)
  ...
  Subroutine advance_angle(angle,delangle)
    Implicit None
    Real(8), Parameter :: MOF_Pi = 3.1415926535897932d0
    REAL(8) angle,delangle
    Real(8), Parameter :: epsc = 1.0e-12
    angle=angle+delangle
    if (angle.lt.-MOF_Pi) then
      angle=angle+2.0_8*MOF_Pi
    endif
