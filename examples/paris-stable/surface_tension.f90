...
module module_surface_tension
  ...
  integer, parameter :: FIT_TYPE=1
  integer, parameter :: spheric_fit_method=1
...
!     if(debug_curvature) print *, "mcount",  method_count
  ...
  subroutine curvature_pop_pass1(i0,j0,k0,kappa,nfound)
    ...
    real(8) :: kappasign, area3d, kappapred
    ...
    ! Note: a perfect plane slab, one-cell-wide, will have 9 heights and grad C = 0
    ...
    ! if not succesful, continue search if mixed_height is set
    if(mixed_heights) then
       ...
       ! debugging:
       ! bpoints=1d300 ! force error if bpoints are used uninitialized. 
       ...
       mv(2) = mxyz(try(3))
       mv(3) = mxyz(try(1))
       ! Test for at least 7 points + origin centroid
       ! to avoid special 2D degeneracy. 
       if ( nfound > nfound_min + 1)  then  
          ! rotate and shift origin
          !  x_i' = x_k(i)
          !  m'_i = m_k(i)
          ! if( maxval(abs(bpoints(1,:)-origin))>1d-50) call pariserror("unexpected 1st point")
          ...
          if(FIT_TYPE==1) then
             ...
             kappapred = 2.d0*(a(1)*(1.d0+a(5)*a(5)) + a(2)*(1.d0+a(4)*a(4)) - a(3)*a(4)*a(5)) &
                  /(1.d0+a(4)*a(4)+a(5)*a(5))**(1.5d0)
          else if(FIT_TYPE==2) then
             ! print *, " Now in pass 1"
             ! print *, " try vector is ", try
             call spheric_fit(points,weights,nfound,a,fit_success,mv,1)
             !call spheric_fit_no_weight(points,nfound,a,fit_success,mv,1)
             kappapred = a(5)
             kappasign = a(6)
          else
             call pariserror("Unknown fit type")
          endif
          ! Debugging
          ! print *,"FIT: i,j,k,kappapred,kappasign,nposit ", i0,j0,k0,kappapred,kappasign,nposit
          ! print *,"FIT: i,j,k,a(1:4) ", i0,j0,k0,a(1:4)
          if(.not.fit_success) then
             geom_case_count(16) = geom_case_count(16) + 1
             nfound = 16  ! encode the fact that no curvature set and no fit success. 
             ! Curvature kappa stays at its initial (subroutine argument) value
             ! if not succesful, return with positive nfound
             return
          else
             kappa = kappasign*kappapred
             nfound = - nfound  ! encode the fact that mixed-heights were used 
             return
          endif  ! .not. fit_success
       endif !  nfound > nfound_min  
       ! Encode the failure of the mixed-height method indicating the number of positions found.
       nfound = nfound + 1
       return
    end if
    ! Encode the resulting status (Nine heights not found and mixed_heights not set)
    nfound=nfound_min + 2
  ...
  subroutine curvature_pop_pass2(i0,j0,k0,kappa,nfound)
    ...
    integer :: m,n,l,i,j,k
    logical :: fit_success 
    integer :: s,c(3),nposit,try(3)
    
    real(8) :: points(NPOS,3),origin(3),a(6)
    real(8) :: fit(NPOS,3),weights(NPOS)
    real(8) :: centroid(3),mxyz(3),mv(3),stencil3x3(-1:1,-1:1,-1:1)
    real(8) :: wg, kappasign, kappapred
    ...
    if(nposit < 6) then
       ...
       if(FIT_TYPE==1) then
          ...
          ! call plot_fit_points(points,origin,try,nposit,i0,j0)
          ! No need to add the 0-origin because the i0,j0,k0 point is in the 
          ! " do m,n,l " loop above.
          call spheric_fit(points,weights,nposit,a,fit_success,mv,2)
          ! call spheric_fit_no_weight(points,nposit,a,fit_success,mv,2)
          ! Debugging
          !         print *, " Now in pass 2"
          !         print *, " try vector is ", try
          ! call plot_sphere_points(a,origin,try,i0,j0)
          kappapred = a(5)
          kappasign = a(6)
       else
          call pariserror("Unknown fit type")
  ...
  subroutine plot_fit_points(points,origin,try,nposit,i0,j0)
    implicit none
    integer :: pass=0
    integer, intent(in):: nposit,i0,j0,try(3)
    integer :: i
    real(8), intent(in) :: points(NPOS,3),origin(3)
    real(8) :: centroid_scaled(3), deltax,points_orig_coord(3)
    print *, "current cell center coordinates x(i0),y(j0)", x(i0),y(j0),try
    if(pass==0) then  ! do this the first time only
       deltax=dx(nx/2)
       OPEN(UNIT=81,FILE=TRIM(out_path)//'/fitpoints.txt')
       do i=1,nposit
! revert the "try" rotation
          points_orig_coord(try(2)) = points(i,1) + origin(try(2))
          points_orig_coord(try(3)) = points(i,2) + origin(try(3))
          points_orig_coord(try(1)) = points(i,3) + origin(try(1))
          centroid_scaled = points_orig_coord*deltax
          centroid_scaled(1) = centroid_scaled(1) + x(i0) 
          centroid_scaled(2) = centroid_scaled(2) + y(j0) 
          write(81,'(5(E15.8,1X))') centroid_scaled(1:2),  &
               points(i,:)
       enddo
       CLOSE(81)
       pass=1
       return
!    else 
!       print *, "PFP: already called"
    endif
  end subroutine plot_fit_points

  subroutine plot_sphere_points(a,xo,try,i0,j0)
    implicit none
    integer :: pass=0
    integer, intent(in):: i0,j0,try(3)
    real(8), intent(in) :: xo(3)
    real(8), intent(in) :: a(6)
    real(8) :: xcenter(3),deltax, radcurv
    if(pass==0) then  ! do this the first time only
       print *, "current cell center coordinates x(i0),y(j0)", x(i0),y(j0)
       deltax=dx(nx/2)
       OPEN(UNIT=81,FILE=TRIM(out_path)//'/sphere_points.txt')
! test if assumptions met
       if (try(3) /= 3) call pariserror (" unexpected try vector ")
       radcurv = a(4)*deltax
       xcenter(try(2)) = a(1) + xo(try(2))
       xcenter(try(3)) = a(2) + xo(try(3))
       xcenter(try(1)) = a(3) + xo(try(1))
       xcenter = xcenter*deltax
       xcenter(1) = xcenter(1) + x(i0)
       xcenter(2) = xcenter(2) + y(j0) 
       write(81,'(5(E15.8,1X))') xcenter(1:2),radcurv,x(i0),y(j0)
       CLOSE(81)
       pass=1
       return
    endif
  end subroutine plot_sphere_points

! Wrapper to call spheric fit when the origin is in the sphere

  subroutine spheric_fit_origin(fit,weights,np,ainout,fit_success,mv)
    implicit none
    integer, intent(in) :: np
    real(8), intent(inout)  :: fit(NPOS,3), weights(NPOS)
    real(8), intent(in)  :: mv(3)
    real(8), intent(inout) :: ainout(6)
    logical, intent(out) :: fit_success

    integer :: np1,pass=0

    if(np>=NPOS) call pariserror("spheric_fit : too many points")
    np1=np+1
    fit(np1,:) = [0.d0,0.d0,0.d0]
    weights(np1) = 1d0
    call spheric_fit(fit,weights,np1,ainout,fit_success,mv,pass)
  end subroutine spheric_fit_origin

! Spheric fit subroutine by Serena Costanzo

  subroutine spheric_fit(fit,weights,np,ainout,fit_success,mv,pass)
    implicit none
    ! a(1:3) = xc
    ! a(4) = radcurv
    ! a(5) = kappa
    ! a(6) = kappasign
    integer, intent(in) :: np,pass
    real(8), intent(in)  :: mv(3),fit(NPOS,3)
    real(8), intent(in)  :: weights(np)
    real(8), intent(inout) :: ainout(6)
    logical, intent(out) :: fit_success
    real(8) :: m(4,4), invm(4,4)
    real(8) :: a(4),rhs(4)
    real(8) :: a3(4),rad_alt,inv3x3(3,3),m3x3(3,3),det3x3
    integer :: ifit, im, jm
    logical :: inverse_success
    real(8) :: x1,x2,x3,y1,y2,y3,z1,z2,z3,sum_x1,sum_x2,sum_y1,sum_y2,sum_z1,sum_z2
    real(8) :: sum_w
    real(8) :: R2,wg,radcurv

    fit_success=.false.
    a = 0.d0; m = 0.d0; rhs = 0.d0
    sum_x1 = 0.d0; sum_x2 = 0.d0
    sum_y1 = 0.d0; sum_y2 = 0.d0 
    sum_z1 = 0.d0; sum_z2 = 0.d0
    sum_w = sum(weights)
    ...
    do ifit=1, np
       ...
       m(1,1) = m(1,1) - 2.d0*sum_w*x2*wg 
       m(2,2) = m(2,2) - 2.d0*sum_w*y2*wg 
       m(3,3) = m(3,3) - 2.d0*sum_w*z2*wg 

       m(1,2) = m(1,2) - 2.d0*sum_w*y1*x1*wg 
       m(1,3) = m(1,3) - 2.d0*sum_w*z1*x1*wg 
       m(2,3) = m(2,3) - 2.d0*sum_w*z1*y1*wg 
       m(4,1) = m(4,1) + 2.d0*x1*wg/sum_w
       m(4,2) = m(4,2) + 2.d0*y1*wg/sum_w
       m(4,3) = m(4,3) + 2.d0*z1*wg/sum_w

       rhs(1) = rhs(1) - sum_w*x3*wg - sum_w*x1*y2*wg - sum_w*x1*z2*wg 
       rhs(2) = rhs(2) - sum_w*y3*wg - sum_w*y1*x2*wg - sum_w*y1*z2*wg 
       rhs(3) = rhs(3) - sum_w*z3*wg - sum_w*z1*x2*wg - sum_w*z1*y2*wg 

    end do

    m(1,1) = m(1,1) + 2.d0*sum_x1*sum_x1
    m(2,2) = m(2,2) + 2.d0*sum_y1*sum_y1
    m(3,3) = m(3,3) + 2.d0*sum_z1*sum_z1

    m(1,2) = m(1,2) + 2.d0*sum_y1*sum_x1
    m(1,3) = m(1,3) + 2.d0*sum_z1*sum_x1
    m(2,3) = m(2,3) + 2.d0*sum_z1*sum_y1

    m(2,1) = m(1,2)
    m(3,1) = m(1,3)
    m(3,2) = m(2,3)

    m(4,4) = 1d0  ! corrects error that gave sum_wg

    R2 =  sum_x2 + sum_y2 + sum_z2
    rhs(1) = rhs(1) + R2*sum_x1
    rhs(2) = rhs(2) + R2*sum_y1
    rhs(3) = rhs(3) + R2*sum_z1
    rhs(4) = R2/sum_w

    ! Solve linear system
    if(spheric_fit_method==1) then 
       ! solve 3x3 system
       m3x3=m(1:3,1:3)
       call FindInverse3x3(m3x3,inv3x3,det3x3,inverse_success,pass,np)
       if(inverse_success) then 
          a3(1:3) = matmul(inv3x3,rhs(1:3))
          a3(4) = - m(4,1)*a3(1) - m(4,2)*a3(2) - m(4,3)*a3(3) + rhs(4)
          rad_alt =  sqrt( a3(4) + a3(1)*a3(1) + a3(2)*a3(2) + a3(3)*a3(3) )
          fit_success = .true.
       else
          ! debugging
          ! call pariserror("failure of 3x3 system")
          return
       end if
       ainout(1:3) = a3(1:3)
       ainout(4) = rad_alt
       ainout(5) = 2d0/rad_alt
       ainout(6) = sign(1.d0,a3(1)*mv(1) + a3(2)*mv(2) + a3(3)*mv(3))
       return
       ! print *,"determinant=",det3x3
       ! Print inverse matrix before system solution
       ...
       !        enddo
       !       stop "debugging"
    else if(spheric_fit_method==2) then
       call FindInverseMatrix(m,invm,4,inverse_success)
       if ( inverse_success ) then
          do im=1,4
             do jm=1,4
          ...
          call pariserror("no inverse success")
       end if
       radcurv = sqrt( a(4) + a(1)*a(1) + a(2)*a(2) + a(3)*a(3) )
       ! ***
       !    Test difference
       print *,"solver difference, determinant,pass",abs(radcurv-rad_alt),det3x3,pass
       ainout(1:3) = a(1:3)
       ainout(4) = radcurv
       ainout(5) = 2d0/radcurv
       ainout(6) = sign(1.d0,a(1)*mv(1) + a(2)*mv(2) + a(3)*mv(3))
       return
    else
       call pariserror("unknown spheric_fit method")
    endif ! spheric_fit_method
  end subroutine spheric_fit

  subroutine spheric_fit_no_weight(fit,np,ainout,fit_success,mv,pass)
    implicit none
    ! a(1:3) = xc
    ! a(4) = radcurv
    ! a(5) = kappa
    ...
    integer, intent(in) :: np,pass
    real(8), intent(in)  :: mv(3),fit(NPOS,3)
    real(8), intent(out) :: ainout(6)
    logical, intent(out) :: fit_success
    real(8) :: m(4,4), invm(4,4) ! ,fit(NPOS,3)
    real(8) :: a(4),rhs(4)
    real(8) :: a3(4),rad_alt,inv3x3(3,3),m3x3(3,3),det3x3
    integer :: ifit, im, jm
    logical :: inverse_success
    ...
    do ifit=1, np
       ...
       m(1,1) = m(1,1) - 2.d0*np*x2 
       m(2,2) = m(2,2) - 2.d0*np*y2 
       m(3,3) = m(3,3) - 2.d0*np*z2 

       m(1,2) = m(1,2) - 2.d0*np*y1*x1 
       m(1,3) = m(1,3) - 2.d0*np*z1*x1 
       m(2,3) = m(2,3) - 2.d0*np*z1*y1 
       m(4,1) = m(4,1) + 2.d0*x1/np
       m(4,2) = m(4,2) + 2.d0*y1/np
       m(4,3) = m(4,3) + 2.d0*z1/np

       rhs(1) = rhs(1) - np*x3 - np*x1*y2 - np*x1*z2 
       rhs(2) = rhs(2) - np*y3 - np*y1*x2 - np*y1*z2 
       rhs(3) = rhs(3) - np*z3 - np*z1*x2 - np*z1*y2 
    ...
    if(spheric_fit_method==1) then 
       ...
       ! ***
       !    Test difference
       ! print *,"solver difference, determinant,pass",abs(radcurv-rad_alt),pass,rad_alt,radcurv
       ainout(1:3) = a(1:3)
       ainout(4) = radcurv
       ainout(5) = 2d0/radcurv
       ainout(6) = sign(1.d0,a(1)*mv(1) + a(2)*mv(2) + a(3)*mv(3))
!        print *,"signs",sign(1.d0,a(1)*mv(1) + a(2)*mv(2) + a(3)*mv(3)), &
!             sign(1.d0,a3(1)*mv(1) + a3(2)*mv(2) + a3(3)*mv(3))
       return
    else
       call pariserror("unknown spheric_fit method")
    endif ! spheric_fit_method
  end subroutine spheric_fit_no_weight
  subroutine FindInverse3x3(m,invm,determinant,inverse_success,pass,np)
    implicit none
    real(8), intent(in) :: m(3,3)
    real(8), intent(out) :: invm(3,3), determinant
    real(8) :: minor(3,3),test(3,3)
    integer, intent(in), optional :: pass, np
    logical, intent(out) :: inverse_success
    integer :: i,j,p,n

    if(.not.present(pass)) then 
       p=0
    else
       p=pass
    endif
    if(.not.present(np)) then
       n=-1
    else
       n=np
    endif
    ...
    minor(1,1)=m(2,2)*m(3,3) - m(2,3)*m(3,2)
    minor(2,1)=m(1,2)*m(3,3) - m(1,3)*m(3,2)
    minor(3,1)=m(1,2)*m(2,3) - m(2,2)*m(1,3)

    minor(1,2)=minor(2,1)
    minor(2,2)=m(1,1)*m(3,3) - m(3,1)*m(1,3)
    minor(3,2)=m(1,1)*m(2,3) - m(2,1)*m(1,3)

    minor(1,3)=minor(3,1)
    minor(2,3)=minor(3,2)
    minor(3,3)=m(1,1)*m(2,2) - m(1,2)*m(2,1)

    determinant=m(1,1)*minor(1,1)-m(2,1)*minor(2,1)+m(3,1)*minor(3,1)
    if(abs(determinant).lt.EPS_GEOM.and.debug_curvature) then 
       print *, "det, pass, np", determinant, p, n
       call pariserror("small determinant")
       return
    endif
! Use the transpose. 
    do i=1,3
       do j=1,3
          invm(i,j) = (-1)**(i+j)*minor(j,i)/determinant
       enddo
    enddo
    if(debug_curvature) then
       ...
       enddo
       if (maxval(abs(test)).gt.1d-10) then 
          print *, "max error of matrix inversion > 1d-10:", maxval(abs(test))
          call pariserror("failed matrix inversion")
       endif
    endif
    inverse_success = .true.
  end subroutine FindInverse3x3
    ...
    subroutine parabola_fit_with_rotation(bfit,fit,weights,mv,nposit,a,kappasign,fit_success)
      ...
      if(do_rotation) then
         ...
         enddo
         if(debug_curvature) then
! apparently due to a small normal mv 
            if(min(norm(1),norm(2),norm(3)) < TINY**2) then
!               print *," mv = ", mv
!               print *," norm = ", norm
!               stop
!            endif
               call pariserror("small rotation matrix in curvature")
            endif
            invm = transpose(ev)
    ...
    SUBROUTINE FindInverseMatrix(matrix,inverse,n,inverse_success)
      ...
      if(debug_curvature) then
         ...
         do i=1,n
            test(i,i) = test(i,i) - 1d0
         enddo
         if (maxval(abs(test)).gt.1d-8) then 
            print *, "max error of matrix inversion > 1d-8:", maxval(abs(test))
            call parismessage("failed matrix inversion")
         endif
         deallocate(test)
      endif
      inverse_success = .true.
...
!     end subroutine LineCenter
    ...
    function ind_pos_sorted_add (centroid, points, bpoints, n)
      ...
      real(8), intent(inout) :: bpoints(NPOS,3)
      integer :: i,j,ni,c
      real(8) :: d2,d1
      logical :: reject
      bpoints(1,:) = centroid
