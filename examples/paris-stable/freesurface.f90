...
!---------------------------------------------------------------------------------------------------
! This is a straight copy of NewSolver. The idea is to not clutter NewSolver 
! with all the FreeSurface flags and tests,
...
!===================================================================================================================================
subroutine setuppoisson_fs_heights(utmp,vtmp,wtmp,umask,vmask,wmask,rho,dt,coeff,bub_id)
  use module_grid
  use module_freesurface
  use module_IO
  use module_VOF
  implicit none
  include 'mpif.h'
  real(8), dimension(imin:imax,jmin:jmax,kmin:kmax), intent(in) :: utmp,vtmp,wtmp,umask,vmask,wmask
...
10  format(6e14.5)
    ...
    do k=ks,ke; do j=js,je; do i=is,ie
       if (vof_phase(i,j,k)==0 .and. (.not.implode_flag(bub_id(i,j,k))) ) then
          ...
          coeff(i,j,k,1) = 2.d0*dt*umask(i-1,j,k)/((dx(i)+x_mod(i-1,j,k))*x_mod(i-1,j,k)*rho)
          coeff(i,j,k,2) = 2.d0*dt*umask(i  ,j,k)/((dx(i)+x_mod(i  ,j,k))*x_mod(i  ,j,k)*rho)
          coeff(i,j,k,3) = 2.d0*dt*vmask(i,j-1,k)/((dy(j)+y_mod(i,j-1,k))*y_mod(i,j-1,k)*rho)
          coeff(i,j,k,4) = 2.d0*dt*vmask(i,j  ,k)/((dy(j)+y_mod(i,j  ,k))*y_mod(i,j  ,k)*rho)
          coeff(i,j,k,5) = 2.d0*dt*wmask(i,j,k-1)/((dz(k)+z_mod(i,j,k-1))*z_mod(i,j,k-1)*rho)
          coeff(i,j,k,6) = 2.d0*dt*wmask(i,j,k  )/((dz(k)+z_mod(i,j,k  ))*z_mod(i,j,k  )*rho)
          coeff(i,j,k,7) = sum(coeff(i,j,k,1:6))
...
subroutine setuppoisson_fs_hypre(utmp,vtmp,wtmp,umask,vmask,wmask,rho,dt,coeff,height,kap,bub_id)
  use module_grid
  use module_BC
  use module_freesurface
  use module_IO
