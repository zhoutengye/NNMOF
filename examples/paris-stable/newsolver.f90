...
subroutine NewSolver(A,p,maxError,beta,maxit,it,ierr,norm)
	...
	elseif (doGPU) then
!~ 		write(*,*) 'heeeeeere'
		call NewSolver_CUDA(A,p,maxError,beta,maxit,it,ierr,norm,tres2)
		print*, "tres2 = ", tres2, "overm max =",maxError
#endif
    else
...
!*** Linfinity norm
      else if(norm==100) then
         do k=ks+1,ke-1; do j=js+1,je-1; do i=is+1,ie-1
            res2=max(res2,abs(-p(i,j,k) * A(i,j,k,7) +                       &
                 A(i,j,k,1) * p(i-1,j,k) + A(i,j,k,2) * p(i+1,j,k) +         &
                 A(i,j,k,3) * p(i,j-1,k) + A(i,j,k,4) * p(i,j+1,k) +         &
         ...
         mask(is+1:ie-1,js+1:je-1,ks+1:ke-1)=.false.
         do k=ks,ke; do j=js,je; do i=is,ie
            if(mask(i,j,k)) res2=max(res2,abs(-p(i,j,k) * A(i,j,k,7) +                       &
                 A(i,j,k,1) * p(i-1,j,k) + A(i,j,k,2) * p(i+1,j,k) +         &
                 A(i,j,k,3) * p(i,j-1,k) + A(i,j,k,4) * p(i,j+1,k) +         &
                 A(i,j,k,5) * p(i,j,k-1) + A(i,j,k,6) * p(i,j,k+1) + A(i,j,k,8) ))
         enddo; enddo; enddo
...
!*** Invalid value of norm (NormOrder) 
      else
         call pariserror("invalid norm")
      endif
  enddo
  if(rank==0.and.recordconvergence) close(89)
contains
  subroutine catch_divergence(res2,ierr)
    ...
    if ((res2*npx*npy*npz)>1.d16 ) then
       if(rank<=30) print*,'Poisson solver diverged after',it,'iterations at rank ',rank
       call pariserror("newsolver error")
    else if (res2 .ne. res2) then 
       if(rank<=30) print*, 'it:',it,'Poisson residual value is invalid at rank', rank
       call pariserror("newsolver error")
    else
       ierr=0
    endif
