...
module module_VOF 
  ...
  real(8) :: EPSC  = 1.d-8  ! for clipping vof and setting flags

  real(8) :: IniWaveAmp ! For initialisation of height test 
  integer :: nwavenumber ! For initialisation of height test 
...
!=================================================================================================
  ...
  subroutine ReadVOFParameters
    ...
    namelist /vofparameters/ vofbdry_cond,test_type,VOF_advect,refinement, &
         ...
         cwg_threshold,EPSC,staggeredmom, LinInterp,IniWaveAmp,nwavenumber
    ...
    EPSC = 1.d-12
    staggeredmom = .true.
    IniWaveAmp = dx(nx/2+2)  ! For initialisation of height test
    nwavenumber = 1
    ...
    enddo
    ! non-periodic VOF BC woud be overwritten by MPI-generated periodic BC so stop the program. 
    do i=1,6
       if(vofbdry_cond(i) /= 'periodic'.and.bdry_cond(i)==1) call pariserror("inconsistent BC")
    enddo
...
!
  subroutine initialize_VOF
    ...
    if(.not.DoFront2VOF) call ReadVOFParameters
! Check grid
    ...
    if (DoMOMCONS) then
      ...
      if (VOF_advect=='Dick_Yue') allocate(ucc(imin:imax,jmin:jmax,kmin:kmax))
    endif
    cvof = 0.d0
    vof_flag = 3
    vof_phase = 2
...
!=================================================================================================
  ...
  subroutine initconditions_VOF()
    ...
    if(Rudman) call rudman_init_vof_pre() ! Subgrid vof initialization 
...
!  test_KHI2D
!  test_turb
!  test_cylinder_advection
!  test_parasitic_currents
!
!   + PHASE_CHANGE tests.
!
    if(test_heights.or.test_capwave.or.test_KHI2D) then 
       if(cylinder_dir==0) then
          write(*,*) "IVOF: Warning: cylinder_dir=0 set to 2"
          cylinder_dir=2
       endif
       ipar=cylinder_dir
       ! correct initial wave amplitude to let initial velocity perturbation be constant. 
       if(test_KHI2D) then 
          rr=rho1/rho2; if(rho2<rho1) rr=rho2/rho1
          IniWaveAmp=IniWaveAmp/(sqrt(rr)*PI*nwavenumber)
       endif
...
#ifdef PHASE_CHANGE
       if (ngradT_test) then
          call single_random_drop
          write(*,*)'Running normal distance test'
       endif
       if (super_liquid) test_fsdrop=.true. !inverts droplet to bubble
#endif
       ...
       if (.not.(test_plane)) then
!          if(bdry_cond(1) /= 3 .and. rank==0 ) write(*,*) &
!               "IVOF: Warning: initializes domain entirely to phase 1 (cvof=0)"
! removed warning as not true anymore, the following if ( ... ) statements modify the cvof=0
          cvof=0.d0
          vof_flag=0
       endif
    endif
    ...
    if(Rudman) then
        call rudman_init_vof_post() ! Subgrid to coarse grid averaging of initialized subgrid vof 
    else
  ...
  !=================================================================================================
  !   Generate a single bubbles with the location moved randomly around the domain center up to one 
  !   grid cell in each direction. The radius is also varied by +- one grid cell to the original 
  !   specified value
  !=================================================================================================
  subroutine single_random_drop()
    use module_2phase
    use module_grid
    ...
    dh=dx(is)
    rad(1) = rad(1) + dh*(2.0d0*rand()-1.0d0)
    xc(1)=xLength/2.0d0 + dh*(2.0d0*rand()-1.0d0)
    yc(1)=yLength/2.0d0 + dh*(2.0d0*rand()-1.0d0)
    zc(1)=zLength/2.0d0 + dh*(2.0d0*rand()-1.0d0)
    write(*,'("Droplet generated at ",3e14.5," with radius: ",e14.5)')xc(1),yc(1),zc(1),rad(1)
    
  end subroutine single_random_drop 
...
! x-axis span: 2*rad/(sqrt cdir) = 2*rad*sqrt(1 + e(1)) etc... 
  ...
  function plane2ls(xx,yy,zz)
    ...
    real(8), intent(in) :: xx,zz,yy
#ifdef PHASE_CHANGE
    if (super_vapour) then
       if (perturb_T) then
          plane2ls = plane&
               -n_p(1)*(xx + 0.5d0*epsBL*xLength*(COS(2.0d0*pi*yy/yLength)+COS(2.0d0*pi*zz/zLength)))&
               -n_p(2)*(yy + 0.5d0*epsBL*yLength*(COS(2.0d0*pi*xx/xLength)+COS(2.0d0*pi*zz/zLength)))&
               -n_p(3)*(zz + 0.5d0*epsBL*zLength*(COS(2.0d0*pi*xx/xLength)+COS(2.0d0*pi*yy/yLength)))
       else
 ...
 ! Subgrid rudman momcons is incompatible with VOFI at the present stage
#ifdef HAVE_VOFI
    integer :: i,j,k,itrue
...
#endif
!-------------------------------------------RUDMAN CHANGES-------------------------------------------!
    else if(Rudman) then ! Subgrid level-set to vof computation 
       call ls2vof_refined(lsfunction,ipar,1)
       call ighost_x_fine(vof_flag,ngh,req(1:4)); call MPI_WAITALL(4,req(1:4),sta(:,1:4),ierr)
       call ighost_y_fine(vof_flag,ngh,req(1:4)); call MPI_WAITALL(4,req(1:4),sta(:,1:4),ierr)
       call ighost_z_fine(vof_flag,ngh,req(1:4)); call MPI_WAITALL(4,req(1:4),sta(:,1:4),ierr)
       
       call ghost_x_fine(cvof,ngh,req(1:4)); call MPI_WAITALL(4,req(1:4),sta(:,1:4),ierr)
       call ghost_y_fine(cvof,ngh,req(1:4)); call MPI_WAITALL(4,req(1:4),sta(:,1:4),ierr)
       call ghost_z_fine(cvof,ngh,req(1:4)); call MPI_WAITALL(4,req(1:4),sta(:,1:4),ierr)
       call vof_bc_rudman(cvof,vof_flag,idx_f,fine)
       call ls2vof_refined(lsfunction,ipar,refinement/2)
...
!------------------------------------------END RUDMAN-------------------------------------------------! 
  ...
  function vofi_lsfunction(xyz)
    ...
    if(test_heights.or.test_KHI2D.or.test_capwave) then 
       if(cylinder_dir==0) then
       ...
       vofi_lsfunction=0d0 ! to avoid compiler warnings
    endif
...
subroutine vofandmomsweepsstaggered(tswap,t)
  ...
  if (staggeredmom) then  
     do dir=1,3
        ! Reconstruct momentum and VOF C in the staggered control volumes.
        if (dir.eq.1) then
        ...
        ! Array tmp is now the VOF C in the staggered control volumes.

        tmp_flag = vof_flag   

        if (VOF_advect=='Dick_Yue') then
          call c_mask(tmp, work(:,:,:,2))
          if (dir.eq.1) then
            ucc(imin:imax,jmin:jmax,kmin:kmax) = u(imin:imax,jmin:jmax,kmin:kmax)
          elseif (dir.eq.2) then
            ucc(imin:imax,jmin:jmax,kmin:kmax) = v(imin:imax,jmin:jmax,kmin:kmax)
          elseif (dir.eq.3) then
            ucc(imin:imax,jmin:jmax,kmin:kmax) = w(imin:imax,jmin:jmax,kmin:kmax)
           endif
        endif

        if (MOD(tswap,3).eq.0) then  ! do z then x then y 
           call swpmom_stg(w,tmp,3,work(:,:,:,1),work(:,:,:,2), &
                work(:,:,:,3),momentum,dir,t,ucc)
...
!=================================================================================================
  ...
  ! function that does all the computations and calls to get area of plane using AREA3D
  ! note that the area is rescaled by the cell size squared, i.e a plane normal to a coord direction 
  ! in a mixed cell will have an area of 1.00
  function cell_area(i,j,k)
    implicit none
    real(8) :: cell_area, AREA3D
    integer, intent(in) :: i,j,k
    integer :: i1,j1,k1
    real(8) :: nr(1:3),stencil3x3(-1:1,-1:1,-1:1)
    cell_area=0.d0
    do i1=-1,1; do j1=-1,1; do k1=-1,1
       stencil3x3(i1,j1,k1) = cvof(i+i1,j+j1,k+k1)
    enddo;enddo;enddo
    call mycs(stencil3x3,nr)
    cell_area=AREA3D(nr,cvof(i,j,k))
  end function cell_area

  subroutine get_all_wetted_area
    use module_grid
    implicit none
    real(8), dimension(is:ie,js:je,ks:ke,1:6) :: A_w
    real(8) :: nr(1:3),stencil3x3(-1:1,-1:1,-1:1), AL3D
       ...
       do j=js,je
          do i=is,ie
             if (vof_flag(i,j,k)==2) then
                ...
                enddo;enddo;enddo
                call mycs(stencil3x3,nr)
                call WETTED_CUBE_FACES(nr,cvof(i,j,k),A_w(i,j,k,1:6))
             endif
          enddo
       enddo
    enddo
    
  end subroutine get_all_wetted_area

  subroutine staggered_cut_gen(cstag,i,j,k,theta,d,limit)
  use module_grid
  implicit none
  real(8), dimension(imin:imax,jmin:jmax,kmin:kmax), intent(in) :: cstag
  real(8), intent(in) :: limit
  real(8) :: theta, test
  integer :: i,j,k,d
  integer :: i0,j0,k0
  real(8), dimension(-1:1,-1:1,-1:1) :: stencil3x3
  real(8) :: n_stag(3), alpha, al3d

  theta = dxh(is)

  !call get_staggered_fractions(cstag,d) ! Ouch, a bit expensive! 

  do i0=-1,1; do j0=-1,1; do k0=-1,1
     stencil3x3(i0,j0,k0) = cstag(i+i0,j+j0,k+k0)
  enddo;enddo;enddo
  call mycs(stencil3x3,n_stag)

  if (ABS((ABS(n_stag(1))+ABS(n_stag(2))+ABS(n_stag(3)))-1.0d0) > 1.0d-10) then
     write(*,*)'Normals not normalised'
     write(*,'("Normals: ",3e14.5)')n_stag(1:3)
     call pariserror('Normals not normalised')
  else
     alpha=al3d(n_stag,cstag(i,j,k))
     if (ABS(n_stag(d))>1.0d-12) then
        test = (alpha - (n_stag(1)+n_stag(2)+n_stag(3)-n_stag(d))/2.0d0)/n_stag(d)
        if (test>1.0d0) test = (1.0d0 - limit)
        if (test<limit) test = limit
        theta = dxh(i)*test
     else
        theta = 0.5d0*dxh(i) !set to standard half length if no cut in staggered cell can be found
     endif
  endif
  
end subroutine staggered_cut_gen
...
!=================================================================================================
  subroutine output_VOF_VTKSP(nf,i1,i2,j1,j2,k1,k2)
    ...
    real(8) :: cfiltered
    !----Add VOF details output
    integer :: i0,j0,k0
    real(8), dimension(i1:i2,j1:j2,k1:k2) :: alpha 
    real(8) :: nr(3), stencil3x3(-1:1,-1:1,-1:1) ,al3d
    !---
    ...
    write(8,15) xh(i1-1),yh(j1-1),zh(k1-1)
...
16  format('SPACING ',F16.8,F16.8,F16.8)
    ...
    write(8,19)(i2-i1)*(j2-j1)*(k2-k1)
    write(8,17)'VOF'
    write(8,18)
19  format('CELL_DATA ',I17)
...
20  format('VECTORS normals double')
write(8,20)
...
do k=k1,k2-1; do j=j1,j2-1; do i=i1,i2-1
   ...
   enddo;enddo;enddo
   call mycs(stencil3x3,nr)
   alpha(i,j,k) = al3d(nr,cvof(i,j,k))
   write(8,310) nr(1),nr(2),nr(3)
enddo; enddo; enddo
...
15  format('POINTS ',I17,' double' )
    ...
    do k=k1,k2; do j=j1,j2; do i=i1,i2;
      write(8,320) xh(i-1),yh(j-1),zh(k-1)
    enddo; enddo; enddo
