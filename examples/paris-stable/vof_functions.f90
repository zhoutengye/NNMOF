...
!===============================================================================
! DESCRIPTION OF SUBROUTINE WETTED_AREA_FACE:
...
SUBROUTINE WETTED_AREA_FACE(nr,cc,f_w)
  ...
  REAL(8) :: al,LIMIT,AL3D,A_tri,A_quad,tri_side(1:2)

  LIMIT = 1.0d-10
  f_w = 0.0d0
  cut = -1.0d-10
  al=AL3D(nr,cc) ! Get alpha for plane

  do dir = 1,3 ! Loop for face normal direction
     do f=0,1 ! Loop for pos and neg face in direction dir (x-, x+, y- ,y+ etc)
        f_id=dir+f*3
        roots=0
        do rt_d=1,3
           if (rt_d==dir) then
              ...
              call other_index(dir,rt_d,oi)
              do rt_f=0,1
                 if (ABS(nr(rt_d))>LIMIT) then
                    cut(rt_d,rt_f) = ( al - REAL(f)*nr(dir) - REAL(rt_f)*nr(oi) )/nr(rt_d)
                    if ( cut(rt_d,rt_f)>LIMIT .and. cut(rt_d,rt_f)<(1.0d0-LIMIT)  ) then
                       roots=roots+1
                       rtf_id(roots) = rt_f
                       rtd_id(roots) = rt_d
                    endif
        ...
        enddo
        ! Get area
        if (roots==0) then ! Case 1: No intercepts
           if ( 1.d3*nr(dir)*REAL(2*f-1)<0.d0 ) then
              f_w(f_id)=1.d0
           else
              f_w(f_id)=0.d0
           endif
        elseif (roots==2) then ! Case 2: Two intercepts
           if ( rtd_id(1)==rtd_id(2) ) then ! Case 2a: Opposite faces
              A_quad= 0.5d0*(cut(rtd_id(1),rtf_id(1)) + cut(rtd_id(2),rtf_id(2)))
              ! Determine wetted/unwetted portion using normal
              if (nr(rtd_id(1))>0.d0) then
                 f_w(f_id)=A_quad
              else
                 f_w(f_id)=1.0d0-A_quad
              endif
           else ! Case 2b: adjacent faces
              do i=1,2
                 if (rtf_id(3-i)==1) then
                    tri_side(i)=1.0d0-cut(rtd_id(i),rtf_id(i))
                 else
                    tri_side(i)=cut(rtd_id(i),rtf_id(i))
                 endif
              enddo
              A_tri=0.5d0*tri_side(1)*tri_side(2)
              ! Determine wetted/unwetted portion using normals
              ! Use largest normal
              if ( ABS( nr(rtd_id(1)) ) > ABS( nr(rtd_id(2)) ) ) then
                 chk_rt=1
              else
                 chk_rt=2
              endif
              if ( 1.d3 * nr(rtd_id(chk_rt)) * REAL(2*rtf_id(3-chk_rt)-1) < 0.d0 ) then
                 f_w(f_id)=A_tri
              else
                 f_w(f_id)=1.0d0-A_tri
              endif
           endif
        else
           write(*,'("Should have 0 or 2 intercepts, but found ",I5," intercepts.")')roots
           call pariserror('Should have 0 or 2 intercepts for wetted area calc.')
        endif
     enddo ! f+ or f-
...
contains 
  subroutine other_index(d,r,oi)
    ...
    do i=1,3
       if (.not.(d==i .or. r==i)) oi=i
    enddo
  end subroutine other_index
END SUBROUTINE WETTED_AREA_FACE

!===============================================================================
! DESCRIPTION OF SUBROUTINE WETTED_CUBE_FACES:
! compute in the unit cube the fraction of each face wetted by the reference
! phase with interface given by the plane
! [nr]*[x] = nr1*x1 + nr2*x2 + nr3*x3 = alpha
! INPUT : normal coefficients in nr(3), volume fraction cc
...
SUBROUTINE WETTED_CUBE_FACES(nr,cc,f_w)
  ...
  f_w(2) = WETTED_FACE(nr(3),nr(1),al)                  ! face at y = 0
  f_w(5) = WETTED_FACE(nr(3),nr(1),al-nr(2))            ! face at y = 1
  f_w(3) = WETTED_FACE(nr(1),nr(2),al)                  ! face at z = 0
  f_w(6) = WETTED_FACE(nr(1),nr(2),al-nr(3))            ! face at z = 1
END SUBROUTINE WETTED_CUBE_FACES

!===============================================================================
! DESCRIPTION OF FUNCTION WETTED_FACE:
! compute the fraction of a cube face wetted by the reference phase with the
! interface given by the plane
! [nr]*[x] = nr1*x1 + nr2*x2 + nr3*x3 = alpha
! INPUT : tangential components of the normal in m1,m2, line constant in al
!-------------------------------------------------------------------------------
FUNCTION WETTED_FACE(m1,m2,al)
  IMPLICIT NONE
  REAL(8), INTENT(IN) :: m1,m2,al
  REAL(8) :: WETTED_FACE
  REAL(8) :: al0,alh,n1,n2,denom
  REAL(8), PARAMETER :: eps0=1.d-50
  INTRINSIC DMAX1,DMIN1,DABS
  ...
  al0 = DMAX1(0.,DMIN1(1.D0, al0/denom))
