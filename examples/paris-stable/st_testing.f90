...
18     format(I8.8,E14.6,E14.6,E14.6,E14.6,E14.6,E14.6,E14.6,E14.6,E14.6,E14.6)   
  ...
  subroutine h_of_KHI2D(timestep,output_time)
    ...
    real(8), dimension(:), allocatable :: local_h, global_h, aan,bbn,aamod
    integer :: ierr
    ...
    IF(first_access) then 
       allocate(local_h(nx), global_h(nx),aan(nx),bbn(nx),aamod(nx))
    ENDIF
    local_KE=0d0
    global_KE=0d0
    local_h=0d0
    ...
    DO i=is,ie
       ...
       IF((rank==1.or.rank==2.or.rank==5.or.rank==6).and.letsdebug) THEN
          OPEN(UNIT=86,FILE=TRIM(out_path)//'/'//TRIM(file_name1)// &
               TRIM(int2text(rank,padding))//'-'//TRIM(int2text(timestep,padding))//'.txt', &
               POSITION='append')
          write(86,18) cvof(i,65,3), cvof(i,66,3), cvof(i,67,3), cvof(i,68,3) 
          CLOSE(86)
...
!       print *, "printing heights"
       local_h(i-Ng) = y(1) - 1d0
!       do j=jmin,js-1
!          if(i.eq.3) print *, "j, height(i,j,k,3),C(i,j,k)",j, height(i,j,k,3),cvof(i,j,k)
!       enddo
       DO j=js,je
!          if(i.eq.3) print *, "j, height(i,j,k,3),C(i,j,k)",j, height(i,j,k,3),cvof(i,j,k)
          diff_vol = dx(i)*dy(j)*dz(k)
          local_KE = local_KE + diff_vol*(0.5d0*(v(i,j,k)+v(i,j+1,k)))**2
          IF((.not.found).and.(height(i,j,k,3)<1d6)) THEN 
             local_h(i-Ng) = y(j) + height(i,j,k,3)*(yLength/Ny) 
             found = .true.
          ENDIF
       ENDDO
!       do j=je+1,jmax
...
!       enddo
    ...
    ! global_h = global_h - yLength/2
    global_KE = global_KE / (xLength*yLength*zLength)
    if (rank==0) then

       a1_coef=0d0;b1_coef=0d0;aan=0.d0;bbn=0.d0
       DO i=1,nx
          do j=1,nx
             aan(j) = aan(j) + global_h(i)*COS(2*PI*j*(X(i))/xLength)*xLength/REAL(nx)
             bbn(j) = bbn(j) + global_h(i)*SIN(2*PI*j*(X(i))/xLength)*xLength/REAL(nx)
          enddo
       ENDDO
       aan = 2*aan/xLength;        bbn = 2*bbn/xLength
       a1_coef = aan(nwavenumber); b1_coef = bbn(nwavenumber)
       aamod = sqrt(aan**2+bbn**2)

       OPEN(UNIT=87,FILE=TRIM(out_path)//TRIM(file_name)//TRIM(int2text(timestep,padding))//'.txt')
       !write(*,*) 'entering print succesfull ', rank
       DO i=1,nx
          write(87,18) x(i+Ng), global_h(i)
17        format(E14.6,E14.6)
       ENDDO
       CLOSE(87)

       if(first_open) then
          ! Create ab coefficients file
       ...
       if(nx<8) stop 'nx too small for KHI2D test'
       write(88,18) output_time, aan(nwavenumber),bbn(nwavenumber), aamod(nwavenumber),aamod(1),&
            aamod(2),aamod(3),aamod(4),aamod(5),aamod(6),aamod(7),aamod(8)
       !                 1             2             3                    4              5
       !        6        7         8      9
       ! Write v**2 file
...
310 format(e14.5,e14.5,e14.5)
  ...
  subroutine output_MPI(index, v_name, out_array)
    ...
    if (ierr /= MPI_SUCCESS) then
       write(*,'("Error opening MPI file, ierr, rank",2I6)')ierr,rank
  ...
  subroutine output5(index,timestep,sub)
    ...
    do k=kse,kee; do j=jse,jee; do i=ise,iee;
       matrix_small(i,j,k)=tmp(i,j,k)
    enddo; enddo; enddo    

    ! Appending curvature variable to *.silo file  
    ierr2 = dbputqv1 (dbfile, 'kapa', 4, 'srm', 3, &
         matrix_small, dims_vof, &
         3, DB_F77NULL, 0, DB_FLOAT, DB_ZONECENT, DB_F77NULL, ierr2)
    ...
    do k=kse,kee; do j=jse,jee; do i=ise,iee;
       matrix_small(i,j,k)=mdot(i,j,k)
    enddo; enddo; enddo    

    ! Appending mass transfer rate to *.silo file  
    ierr2 = dbputqv1 (dbfile, 'mdot', 4, 'srm', 3, &
         matrix_small, dims_vof, &
         3, DB_F77NULL, 0, DB_FLOAT, DB_ZONECENT, DB_F77NULL, ierr2)

    do k=kse,kee; do j=jse,jee; do i=ise,iee;
       matrix_small(i,j,k)=s_v(i,j,k)
    enddo; enddo; enddo    

    ! Appending mass transfer source term to *.silo file  
    ierr2 = dbputqv1 (dbfile, 'svol', 4, 'srm', 3, &
         matrix_small, dims_vof, &
         3, DB_F77NULL, 0, DB_FLOAT, DB_ZONECENT, DB_F77NULL, ierr2)
    ...
    if (type>0) then
       do k=kse,kee; do j=jse,jee; do i=ise,iee;
          if ( i == imin ) then 
             matrix_small(i,j,k)=1.5*u_l(i,j,k)-0.5*u_l(i+1,j,k)
          else 
             matrix_small(i,j,k)=0.5*(u_l(i,j,k)+u_l(i-1,j,k))
          end if ! i
       enddo; enddo; enddo

       ! Appending u_component variable to *.silo file  
       ierr2 = dbputqv1 (dbfile, 'ulve', 4, 'srm', 3, &
            matrix_small, dims_vof, &
            3, DB_F77NULL, 0, DB_FLOAT, DB_ZONECENT, DB_F77NULL, ierr2) 

       do k=kse,kee; do j=jse,jee; do i=ise,iee;
          if ( j == jmin ) then
             matrix_small(i,j,k)=1.5*v_l(i,j,k)-0.5*v_l(i,j+1,k)
          else 
             matrix_small(i,j,k)=0.5*(v_l(i,j,k)+v_l(i,j-1,k))
          end if ! j
       enddo; enddo; enddo

       ! Appending v_component variable to *.silo file  
       ierr2 = dbputqv1 (dbfile, 'vlve', 4, 'srm', 3, &
            matrix_small, dims_vof, &
            3, DB_F77NULL, 0, DB_FLOAT, DB_ZONECENT, DB_F77NULL, ierr2)

       do k=kse,kee; do j=jse,jee; do i=ise,iee;
          if ( k == kmin ) then
             matrix_small(i,j,k)=1.5*w_l(i,j,k)-0.5*w_l(i,j,k+1)
          else 
             matrix_small(i,j,k)=0.5*(w_l(i,j,k)+w_l(i,j,k-1))
          end if ! k
       enddo; enddo; enddo

       ! Appending w_component variable to *.silo file  
       ierr2 = dbputqv1 (dbfile, 'wlve', 4, 'srm', 3, &
            matrix_small, dims_vof, &
            3, DB_F77NULL, 0, DB_FLOAT, DB_ZONECENT, DB_F77NULL, ierr2)
       ...
       do k=kse,kee; do j=jse,jee; do i=ise,iee;
          if ( i == imin ) then 
             matrix_small(i,j,k)=1.5*u_s(i,j,k)-0.5*u_s(i+1,j,k)
          else 
             matrix_small(i,j,k)=0.5*(u_s(i,j,k)+u_s(i-1,j,k))
          end if ! i
       enddo; enddo; enddo

       ! Appending u_component variable to *.silo file  
       ierr2 = dbputqv1 (dbfile, 'usve', 4, 'srm', 3, &
            matrix_small, dims_vof, &
            3, DB_F77NULL, 0, DB_FLOAT, DB_ZONECENT, DB_F77NULL, ierr2) 

       do k=kse,kee; do j=jse,jee; do i=ise,iee;
          if ( j == jmin ) then
             matrix_small(i,j,k)=1.5*v_s(i,j,k)-0.5*v_s(i,j+1,k)
          else 
             matrix_small(i,j,k)=0.5*(v_s(i,j,k)+v_s(i,j-1,k))
          end if ! j
       enddo; enddo; enddo

       ! Appending v_component variable to *.silo file  
       ierr2 = dbputqv1 (dbfile, 'vsve', 4, 'srm', 3, &
            matrix_small, dims_vof, &
            3, DB_F77NULL, 0, DB_FLOAT, DB_ZONECENT, DB_F77NULL, ierr2)

       do k=kse,kee; do j=jse,jee; do i=ise,iee;
          if ( k == kmin ) then
             matrix_small(i,j,k)=1.5*w_s(i,j,k)-0.5*w_s(i,j,k+1)
          else 
             matrix_small(i,j,k)=0.5*(w_s(i,j,k)+w_s(i,j,k-1))
          end if ! k
       enddo; enddo; enddo

       ! Appending w_component variable to *.silo file  
       ierr2 = dbputqv1 (dbfile, 'wsve', 4, 'srm', 3, &
            matrix_small, dims_vof, &
            3, DB_F77NULL, 0, DB_FLOAT, DB_ZONECENT, DB_F77NULL, ierr2)
    endif
#endif
  ...
  subroutine write_master(rootname, step, time, timestep, dosub, sub)
    ...
    character(len=levarnames), dimension(numProcess) :: vec3n, vecn, presn, kapan
#ifdef PHASE_CHANGE
    character(len=levarnames), dimension(numProcess) :: tempn, mdotn, svoln
    character(len=levarnames), dimension(numProcess) :: vecl1n, vecl2n, vecl3n, vecs1n, vecs2n, vecs3n
#endif
    ...
    do m=0,numProcess-1
       ...
       kapan(m+1) = TRIM(file_n)//TRIM(i2t(m,padd))//'.silo:kapa'
#ifdef PHASE_CHANGE
       tempn(m+1) = TRIM(file_n)//TRIM(i2t(m,padd))//'.silo:temp'
       mdotn(m+1) = TRIM(file_n)//TRIM(i2t(m,padd))//'.silo:mdot'
       svoln(m+1) = TRIM(file_n)//TRIM(i2t(m,padd))//'.silo:svol'
       vecl1n(m+1) = TRIM(file_n)//TRIM(i2t(m,padd))//'.silo:ulve'
       vecl2n(m+1) = TRIM(file_n)//TRIM(i2t(m,padd))//'.silo:vlve'
       vecl3n(m+1) = TRIM(file_n)//TRIM(i2t(m,padd))//'.silo:wlve'
       vecs1n(m+1) = TRIM(file_n)//TRIM(i2t(m,padd))//'.silo:usve'
       vecs2n(m+1) = TRIM(file_n)//TRIM(i2t(m,padd))//'.silo:vsve'
       vecs3n(m+1) = TRIM(file_n)//TRIM(i2t(m,padd))//'.silo:wsve'
#endif
    ...
    err = dbputmvar(dbfile, "kappa", 5, numProcess, kapan, lvarnames, &
         vartypes, DB_F77NULL, ierr)
#ifdef PHASE_CHANGE
    err = dbputmvar(dbfile, "Te", 2, numProcess, tempn, lvarnames, &
         vartypes, DB_F77NULL, ierr)      
    err = dbputmvar(dbfile, "mdot", 4, numProcess, mdotn, lvarnames, &
         vartypes, DB_F77NULL, ierr)
    err = dbputmvar(dbfile, "svol", 4, numProcess, svoln, lvarnames, &
         vartypes, DB_F77NULL, ierr)
    if (type>0) then
       err = dbputmvar(dbfile, "ulve", 4, numProcess, vecl1n, lvarnames, &
            vartypes, DB_F77NULL, ierr)

       err = dbputmvar(dbfile, "vlve", 4, numProcess, vecl2n, lvarnames, &
            vartypes, DB_F77NULL, ierr)

       err = dbputmvar(dbfile, "wlve", 4, numProcess, vecl3n, lvarnames, &
            vartypes, DB_F77NULL, ierr)

       err = dbputdefvars(dbfile, 'defvars',7, 1, 'velocity_1',10, &
            DB_VARTYPE_VECTOR, '{ulve,vlve,wlve}', 16, DB_F77NULL, ierr)

       err = dbputmvar(dbfile, "usve", 4, numProcess, vecs1n, lvarnames, &
            vartypes, DB_F77NULL, ierr)

       err = dbputmvar(dbfile, "vsve", 4, numProcess, vecs2n, lvarnames, &
            vartypes, DB_F77NULL, ierr)

       err = dbputmvar(dbfile, "wsve", 4, numProcess, vecs3n, lvarnames, &
            vartypes, DB_F77NULL, ierr)

       err = dbputdefvars(dbfile, 'defvars',7, 1, 'velocity_s',10, &
            DB_VARTYPE_VECTOR, '{usve,vsve,wsve}', 16, DB_F77NULL, ierr)
    endif
#endif
