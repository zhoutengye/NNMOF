...
!    call pariserror("slope limiter not defined");
  ...
  function interpoleWENO5(valdd,vald,valc,valr,valrr,output)
    implicit none
    real(8) :: interpoleWENO5, valdd, vald, valc, valr, valrr, weight_tot, sumweno
    real(8), dimension(1:3) :: weightnorm, weight, p, beta, pw
    real(8) :: epsilon=1.d-6
    integer :: i
    logical :: output
    
    !get interpolation polynomials
    p(1)=1.d0/3.d0*valdd - 7.d0/6.d0*vald + 11.d0/6.d0*valc
    p(2)=-1.d0/6.d0*vald + 5.d0/6.d0*valc + 1.d0/3.d0*valr
    p(3)=1.d0/3.d0*valc + 5.d0/6.d0*valr - 1.d0/6.d0*valrr
    
    !get weighting factors
    beta(1)=13.d0/12.d0*((valdd-2.d0*vald + valc)**2.d0) + 0.25d0*(valdd-4.d0*vald+3.d0*valc)**2.d0
    beta(2)=13.d0/12.d0*((vald -2.d0*valc + valr)**2.d0) + 0.25d0*(vald - valr)**2.d0
    beta(3)=13.d0/12.d0*((valc -2.d0*valr +valrr)**2.d0) + 0.25d0*(3.d0*valc-4.d0*valr+valrr)**2.d0
    pw(1)=1.0d-1; pw(2)=6.0d-1; pw(3)=3.0d-1
    
    weight_tot=0.d0
    do i=1,3
       weight(i)=pw(i)/(beta(i) + epsilon)**2.d0
       weight_tot=weight_tot+weight(i)
    enddo

    do i=1,3
       weightnorm(i)=weight(i)/weight_tot
    enddo

    if (ABS(sum(weightnorm(1:3))-1.d0)>1.d-10) call pariserror('weights not normalized')

    sumweno=0.d0
    do i=1,3
       sumweno = sumweno + weightnorm(i)*p(i)
    enddo
    interpoleWENO5=sumweno
    if (output) then
       open(unit=770,file="WENO5.txt",position="append")

       write(770,'("Int vals:   ",5e14.5)')valdd,vald,valc,valr,valrr
       write(770,'("Poly vals:  ",3e14.5)')p
       write(770,'("Beta vals:  ",3e14.5)')beta
       write(770,'("Weights  :  ",3e14.5)')weight
       write(770,'("Normalized: ",3e14.5)')weightnorm
       write(770,'("Final:      ",e14.5)')interpoleWENO5
       write(770,'(" ")')
       close(770)
    endif
  end function interpoleWENO5
...
module module_flow
  ...
  real(8), dimension(:,:,:), allocatable :: ucc
  ...
  logical :: ZeroReynolds,DoVOF, DoFront, DoFront2VOF, Implicit, hypre, GetPropertiesFromFront
  logical :: FreeSurface, DoLPP = .false.
  ...
  real(8) :: beta,betaDensity, MaxError,MaxErrorDensity,ErrorScaleHYPRE,DivergeTol,TolDens
  logical :: DynamicAdjustPoiTol,SwitchHYPRESolver 
  real(8) :: ResRatioDecreaseErrorScaleHYPRE,ResRatioIncreaseErrorScaleHYPRE
  integer :: ResNormOrderPressure, ResNormOrderDensity
  ...
  logical :: do_energy !switch for energy equation
...
module module_timer
  implicit none
  save
  integer, parameter :: components=20, steps=1000
  ...
  integer :: ierr2
  character(26) :: timer_component(components)
...
!  end subroutine initialize_sizer
  subroutine initialize_timer
    ...
    timer_component(5) = 'VOF Heights calculation'
    !                     12345678901234567890123456
    timer_component(6) = 'VOF Heights communications'
    timer_component(7) = 'VOF Curvature'
    timer_component(8) = 'VOF Surface tension'
    timer_component(9) = 'Momentum Advection'
    timer_component(10) ='Poisson solve for pressure'
    timer_component(11) = 'Output to file'
    timer_component(12) = 'Lagr particles advection'
    timer_component(13) = 'FT GetFront wait'
    timer_component(14) = 'Tag, convert, drop stats'
    timer_component(15) = 'Free Surface bubbles'
    !                      12345678901234567890123456
    timer_component(16) = 'FT color func. Pois solve'
    timer_component(17) = 'FT color communications'
    timer_component(18) = 'FT AdvanceFront2'
    timer_component(19) = 'FT GetFront send'
    timer_component(20) = 'FT Front2grid (SurfTens)'
    else if (rank==npdomain) then
       timer_component(8) = 'Initialisation'
       timer_component(1) = 'Miscellaneous'
       timer_component(2) = 'Correct Volume'
       timer_component(3) = 'Average Front'
       timer_component(4) = 'Regrid Front'
       timer_component(5) = 'Calculate Surf Tension'
       timer_component(6) = 'Wait for Distrib send'
       timer_component(7) = 'Wait for Distrib receive'
       timer_component(9) = 'Calculate Volume'
       timer_component(10)= 'Output to file'       
       timer_component(11)= 'Smooth'
    end if
    times=0d0
...
!    close(122)
  ...
  subroutine my_timer(n)
    ...
    integer, intent(in) :: n
    real(8) :: elapsed_time
    if(rank>0.and.rank /= npdomain) return
  ...
  subroutine wrap_up_timer_0()
    use module_grid
    use module_flow
    implicit none
    include 'mpif.h'
    integer :: n,ierr2,sl(40),i
    real(8) :: totaltime, ZZ
    if(rank>0) return
    ...
    write(123,*) "Fluid (Domain) CPU profiling on processor 0"
    write(123,*) "   "
    sl = reshape([2,9,1,3,10,11],[40],[0])
    do n=1,6
       write(123,'((A),T27," ",f5.1," %",T37,F7.1," sec.")') TRIM(timer_component(sl(n))), &
            & percentage(sl(n)),totaltime*percentage(sl(n))
    enddo
    if(DoVOF) then
       write(123,*) "   "
       write(123,*) "VOF:   "
       write(123,*) "   "    
       sl =  reshape([ 4,5,6,7,8 ],[40],[0]) 
       do n=1,5
          write(123,'((A),T27," ",f5.1," %",T37,F7.1," sec.")') TRIM(timer_component(sl(n))), &
               & percentage(sl(n)),totaltime*percentage(sl(n))
       end do
    ...
    if(FreeSurface) then
       write(123,*) "   "
       write(123,*) "FreeSurface: "
       write(123,*) "   "    
       write(123,'((A),T27," ",f5.1," %",T37,F7.1," sec.")') TRIM(timer_component(15)), &
         & percentage(15),totaltime*percentage(15)
    endif
    
    write(123,*) "   "
    write(123,'("Overall speed Z/np    ",T28,es7.1e2)') ZZ
    write(123,'("Allocated/proc        ",T28,es7.1e2," Gbytes")') alloc_size
    write(123,'("Current/ ini. t. step:",T28,2(I7,1X))') itimestep,iTimeStepRestart
!    write(123,'("Current/ ini. t.     :",T28,2(ES7.0,1X))') end_loop,start_loop
    write(123,'("Elapsed CPU time     :",T28,F7.3,1X,"sec.")') end_loop-start_loop
    write(123,*) "   "
    close(123)
  end subroutine wrap_up_timer_0
  subroutine wrap_up_timer_np()
    use module_grid
    implicit none
    include 'mpif.h'
    integer :: n
    ...
    percentage = 1d2*times/totaltime
    open(unit=123,file='aggregate_timer_stats_np')
    write(123,'("Front CPU profiling on processor npDomain =",I3)') npDomain
    write(123,*) "   "
    do n=1,11
       write(123,'((A),T27," ",f5.1," %",T37,F7.1," sec.")') TRIM(timer_component(n)), &
            & percentage(n),totaltime*percentage(n)
    end do
    write(123,*) "   "
    close(123)
  end subroutine wrap_up_timer_np
...
module module_freesurface
  ...
  logical :: debug=.false., initialize_fs = .false.
...
!=================================================================================================
!=================================================================================================
! module_boil: Contains variables for phase change and the energy equation
!-------------------------------------------------------------------------------------------------
module module_boil
real(8), allocatable, dimension(:,:,:) :: Te, Te_old, s_v, kc, kc_old, rho_cp, rho_cp_old
real(8), allocatable, dimension(:,:,:) :: u_l,u_s,v_l,v_s,w_l,w_s,p_s
real(8), allocatable, dimension(:,:,:) :: energy, energy_old, mdot, dc
real(8), allocatable, dimension(:,:,:) :: Tumask, Tvmask, Twmask
real(8), allocatable, dimension(:) :: Tprof, R_tprof
integer, allocatable, dimension(:,:,:) :: topo_mask
real(8) :: Cp1, Cp2, h_fg, kc1, kc2, T_sat, T0_1, T0_2
real(8) :: qdot, inv_drho, q_set
real(8) :: ec1, ec2, T_ref ! Thermal expansion coeffs and ref temp for Boussinesq
real(8) :: epsT, epsBL
real(8) :: BC_T(1:6) 
real(8) :: t_0
integer :: BDRY_T(1:6)
integer :: phase_calc, iterPcor
integer :: type, adv_method
character(20) :: setup,EtAdvScheme
logical :: super_vapour, diff_tsat_1D, ngradT_test, super_liquid, perturb_T, TdiffMods
logical :: HYPRE_Ps, HYPRE_Timp
...
!          write(89,310) p(i,j,k)
    ...
    ! writing Temperature
    !
    OPEN(UNIT=89,FILE=TRIM(out_path)//'/Te-'//TRIM(int2text(rank,padding))//'-'//TRIM(int2text(iout,padding))//'.txt')
    k=(ks+ke)/2

    do j=js,je
       do i=is,ie
          write(89,'(3e14.5)') x(i),y(j),Te(i,j,k)
       enddo 
       WRITE(89,*) " "
    enddo
    close(unit=89)
...
module module_turb_inflow
   use module_grid
   use module_2phase
   implicit none
   ! the dimension is the whole left surface, namely Nyt by Nzt
   real(8), dimension(:,:), allocatable :: ut,vt,wt
   !  filter parameters 
   integer :: nfx,nfy,nfz,lnx,lny,lnz
   !  parameters to define the region to add turbulent fluctuations 
   real(8) :: ymin_tf, ymax_tf
   ! computed based ymin_tf and ymax_tf
   integer :: jmin_tf, jmax_tf 
   !  filter coeffecients
   real(8), dimension(:), allocatable :: bx,by,bz
   integer :: ierr1
   ! the mean value of mean velocity profile in y direction
   real(8) :: umean 
   ! for 3D random number space
   real(8), dimension(:,:,:), allocatable :: xu,xv,xw 
   ! the 2D correlated field
   real(8), dimension(:,:), allocatable :: yu,yv,yw
   ! intensity of the turbulent fluctuations 
   real(8) :: inlet_turb_intensity
   integer :: turb_condition,data_length
contains
   
   subroutine initialize_turb_inflow
      use module_grid
      implicit none
      integer :: ierr1
      jmin_tf = INT((ymin_tf + dble(Ng)*dy(2))/dy(2)) + 1
      jmax_tf = INT((ymax_tf + dble(Ng)*dy(2))/dy(2))

      ! allocation  
      allocate(bx(-nfx:nfx),by(-nfy:nfy),bz(-nfz:nfz),STAT=ierr1)
      allocate(ut(1:Nyt,1:Nzt),vt(1:Nyt,1:Nzt),wt(1:Nyt,1:Nzt),STAT=ierr1)
      allocate(xu(-nfx:nfx,-nfy+jmin_tf:jmax_tf+nfy,-nfz+1:Nzt+nfz),&
               xv(-nfx:nfx,-nfy+jmin_tf:jmax_tf+nfy,-nfz+1:Nzt+nfz),&
               xw(-nfx:nfx,-nfy+jmin_tf:jmax_tf+nfy,-nfz+1:Nzt+nfz),STAT=ierr1)

      xu=0.d0; xv=0.d0; xw=0.d0

      ! calucalate filter coefficients for 3 directions
      call rand3d(xu)
      call rand3d(xv)
      call rand3d(xw)
      
      ! calucalate filter coefficients for 3 directions
      call calcoeff(bx,nfx,lnx)
      call calcoeff(by,nfy,lny)
      call calcoeff(bz,nfz,lnz)
      
   end subroutine initialize_turb_inflow

   subroutine Set_meanflow_turb_inflow(time)
      implicit none
      integer :: i,j,k
      real(8),intent(in) :: time  
      real(8) :: NozzleThickness
      real(8), dimension(1:data_length) :: y_exp,u_exp
      real(8) :: yj,uj  
      if (turb_condition == 2) then
         ! readin data
         call read_vel_profile(y_exp,u_exp)
      else 
         y_exp(:) = 0.d0
         u_exp(:) = 0.d0
         yj       = 0.d0
      ...
      ! compute the mean flow, ut is only calculated at the local block
      do j= 1,Nyt    !jmin,jmax
         do k= 1,Nzt !kmin,kmax
              !for ut
              ...
              if ( y(j) <= radius_liq_inject ) then 
                 ut(j,k) = uliq_inject & 
                    ...
                    select case (turb_condition)
                       ...
                       ! read from data
                       case (2)
                         !normalize from 0 to 1 for interpretation   
                         yj  = (y(j)-ymin_tf)/(ymax_tf-ymin_tf)
                         ! linear interpoliation 
                         call interp1D(y_exp,u_exp,yj,uj)
                         ut(j,k) = ugas_inject * uj 
                    end select
              else 
                  ut(j,k) = 0.d0 
              end if  !
      ...
      ! calculate the mean value of mean flow velocity in the region that
      ! turbulent fluctuations are added
      ! Note: This works for uniform mesh only 
      umean = 0.d0 
      do j = jmin_tf, jmax_tf
         umean = umean + ut(j,kmin) !ut(j,1)
      end do ! j 
      umean = umean/dble(jmax_tf-jmin_tf+1)

      ! Normalize ut to one and later we will scale it back by umean
      ut(jmin_tf:jmax_tf,:) = ut(jmin_tf:jmax_tf,:)/umean
   end subroutine Set_meanflow_turb_inflow
   
   subroutine ImposeTurbFluc
      implicit none

      integer :: ierr1
      ! only add the section where the gas turbulence would be imposed 
      ! only the subdomain (such as the gas inlet) will be modified
      allocate(yu(jmin_tf:jmax_tf,1:Nzt),&
               yv(jmin_tf:jmax_tf,1:Nzt),&
               yw(jmin_tf:jmax_tf,1:Nzt),STAT=ierr1)
      
      yu(:,:)=0.d0; yv(:,:)=0.d0; yw(:,:)=0.d0
      ! filter data x -> y         
      call filter3deff(xu,yu,bx,by,bz)
      call filter3deff(xv,yv,bx,by,bz)
      call filter3deff(xw,yw,bx,by,bz)
      ! account for one point statistics according to prescribed 
      ! reynolds stress tensor
      ! create another subroutine to evaluate corelaiton tensor
      call transf_turb
      ! boundaries 
      call uflowsi
      ! scale yu
      call scale3d
      ! add the turbulence to ut, vt & wt
      call augument
      ! shift 3D random fields for u,v,w    
      call rand2d(xu)
      call rand2d(xv)
      call rand2d(xw)
      ! deallocate 
      deallocate(yu,yv,yw)
   end subroutine ImposeTurbFluc
   
   !***************************** calcoeff ********************************
   subroutine calcoeff(b,n,ln)
   !***********************************************************************
   !*     calculate filter coefficients. autocorrelation has shape of     *
   !*     gaussian curve with length scale ln*grid spacing                *
   !*     2*n+1 is the length of the filter                               *     
   !*     n is the filter width and ln is the length scale                * 
   !***********************************************************************      
   implicit none
   !include 'filter.inc'
   ...
   integer :: i
   real(8) :: norm
   real(8), dimension(-nfx:nfx),intent(out) :: b 
   real(8),parameter :: pi = 3.14159265358979323846d0 
   !**********************************************************************
   norm=0.0
   do i=-n,n
      b(i)=exp(-pi*i*i/(2*ln*ln))
      norm=norm+b(i)**2 
   end do   
   ...
   !***************************** rand3d **********************************
   subroutine rand3d(xt)
   !***********************************************************************
   !*     initialize a 3d random field. This field will later be filtered *
   !*     this step is only needed once, see also routine rand2d          *      
   !**************************** arguments ********************************
   implicit none      
   real(8), dimension(-nfx:nfx,-nfy+jmin_tf:jmax_tf+nfy,-nfz+1:Nzt+nfz),intent(out) :: xt
   real(8) :: r
   !*************************** local variables ***************************      
   integer :: i,j,k
   !***************** generate random numbers *****************************
   do i=-nfx,nfx
      do j=-nfy+jmin_tf,jmax_tf+nfy
         do k=-nfz+1,Nzt+nfz
            call RANDOM_NUMBER(r)
            xt(i,j,k)= (r*2.d0-1.d0)/0.5774d0 
         end do
      end do
   end do
   !***************** for boundary conditions *****************************
   ! homogenesous for z-direction 
   if (2*nfz+1.gt.Nzt) write(*,*) 'sanity check failed'
   do k=0,-nfz+1,-1
      do i=-nfx,nfx
         do j=-nfy+jmin_tf,jmax_tf+nfy
            xt(i,j,k)=xt(i,j,Nzt+k)
         end do
      end do 
   end do         
   do k=Nzt+1,Nzt+nfz
   ...
   !******************************* rand2D ********************************
   subroutine rand2d(xt)
   !*     shift random data and generate one new slide.                   *
   implicit none
   real(8), dimension(-nfx:nfx,-nfy+jmin_tf:jmax_tf+nfy,-nfz+1:Nzt+nfz),intent(inout) :: xt
   real(8) :: r
   !************************* local variables ***************************** 
   integer :: i,j,k
   ! shift data x(i,j,k) = x(i+1,j,k)
   do k=-nfz+1,Nzt+nfz
      do j=-nfy+jmin_tf,jmax_tf+nfy
         do i=-nfx,nfx-1
            xt(i,j,k)=xt(i+1,j,k)
   ...
   ! fill x(i+1,j,k) with random data           
   do k=-nfz+1,Nzt+nfz
      do j=-nfy+jmin_tf,jmax_tf+nfy
         call RANDOM_NUMBER(r)
         xt(nfx,j,k)= (r*2.d0-1.d0)/0.5774d0
      end do
   end do
   ...
   subroutine filter3deff(xt,yt,bx,by,bz)
   !***********************************************************************
   !*     filtering of the data. input :x output y                        *
   !*     filter coefficients: a  filter length: 2*( nx,ny,nz )+1         *
   !**************************** arguments ********************************
   implicit none ! add intent
   real(8), dimension(-nfx:nfx,-nfy+jmin_tf:jmax_tf+nfy,-nfz+1:Nzt+nfz),intent(in) :: xt
   real(8), dimension(jmin_tf:jmax_tf,1:Nzt), intent(out) :: yt
   real(8), dimension(-nfx:nfx), intent(in) :: bx
   real(8), dimension(-nfy:nfy), intent(in) :: by
   real(8), dimension(-nfz:nfz), intent(in) :: bz
   !******************** local variables **********************************
   integer :: i,j,k,i2,j2,k2
   real(8) :: yx(-nfy+jmin_tf:jmax_tf+nfy,-nfz+1:Nzt+nfz)
   real(8) :: yy(jmin_tf:jmax_tf,-nfz+1:Nzt+nfz)
   !***********************************************************************
   ! in x direction
   do k=-nfz+1,Nzt+nfz
      do j=-nfy+jmin_tf,jmax_tf+nfy
   ...
   do k=1,Nzt
      do j=jmin_tf,jmax_tf
         yt(j,k)=0.0
         do k2=-nfz,nfz
            yt(j,k)=yt(j,k)+yy(j,k+k2)*bz(k2)
         end do         
      end do
   end do
   end subroutine filter3deff
   !***********************************************************************

   !***************    transf_turb ****************************************
   subroutine  transf_turb
   implicit none
   !********************** local variables *********************************
   integer :: j,k
   real(8) :: I,a11,a21,a22,a31,a32,a33
   real(8), dimension(1:data_length) :: y_exp,uu_exp,vv_exp,ww_exp,uv_exp
   real(8) :: yj,uuj,vvj,wwj,uvj 
   !****************************** begin of code ***************************
   ! readin experimental data
   if (turb_condition == 2) then 
      ! readin data 
      call read_turb_corr(y_exp,uu_exp,vv_exp,ww_exp,uv_exp)
   else 
      y_exp(:) = 0.d0
      uu_exp(:) = 0.d0
      vv_exp(:) = 0.d0
   ...
   do j = jmin_tf,jmax_tf 
      do k = 3,Nzt-2          
         ! case 1: constant; case2: exp data
         select case (turb_condition)
            case (1)
               ! use constant
               I = inlet_turb_intensity
               uuj = I**2.d0
               vvj = I**2.d0
               wwj = I**2.d0
               uvj = I**2.d0
               ...
               yj  = 0.d0
            case (2)
               ! normalization for linear interpolation
               yj = (yj-ymin_tf)/(ymax_tf-ymin_tf)
               ! use linear interpretation
               call interp1D(y_exp,uu_exp,yj,uuj)
               call interp1D(y_exp,vv_exp,yj,vvj)
               call interp1D(y_exp,ww_exp,yj,wwj)
               call interp1D(y_exp,uv_exp,yj,uvj)
         end select
         ! define correlation tensor components
         a11 = sqrt(uuj)  
         a21 = 0.d0 !uvj/a11
         a22 = sqrt(vvj-a21**2.d0) !**0.5d0
         a31 = 0.d0
         a32 = 0.d0
         a33 = sqrt(wwj)
         ! don't change the order here !
         yw(j,k) = a31*yu(j,k) + a32*yv(j,k) + a33*yw(j,k)
         yv(j,k) = a21*yu(j,k) + a22*yv(j,k)
         yu(j,k) = a11*yu(j,k) + ut(j,kmin)
      end do
   end do
   end subroutine transf_turb 
   !***************  transf_turb   ***********************************************

   !***************  read_corr_turb   ***********************************************
   subroutine read_turb_corr(y,uu,vv,ww,uv)
      implicit none
      integer :: l
      real(8), dimension(1:data_length) :: y,uu,vv,ww,uv
      ! readin data
      open(unit=10,file="./exp_data/turb_tensor.dat",action='read')
         do l = 1,data_length
            read(10,*) y(l),uu(l),vv(l),ww(l),uv(l)
         end do
      close(10)   
   end subroutine read_turb_corr
   !***************  END read_turb_corr   ***********************************************
   
   !***************  read_vel_profile   ***********************************************
   subroutine read_vel_profile(y,u)
      implicit none
      integer :: l
      real(8), dimension(1:data_length) :: y,u
      ! read experimental data
      open(unit=10,file="./exp_data/vel_profile.dat",action='read')
         do l = 1,data_length
            read(10,*) y(l),u(l)
         end do
      close(10)   
   end subroutine read_vel_profile
   !***************  END read_vel_profile   ***********************************************
      
   !***************  interp1D   ***********************************************
   subroutine interp1D(xv,yv,xi,yi)
   ! define parameters
   implicit none
   ! xv & yv are the experimetal data
   real(8), dimension(:),intent(in) :: xv,yv
   ! define xi to find yi
   real(8), intent(in)  :: xi
   real(8), intent(out) :: yi
   integer  :: i
   ! loop to find the interval linear interpolation
   i = 1
   do while (1 .GT. 0)
   if (xi .LE. (xv(i+1))) then
      exit
   endif
   i = i + 1
  end do
  ! linear interpolation
  yi = yv(i) + (yv(i+1)-yv(i))/(xv(i+1)-xv(i))*(xi-xv(i))
  ...
  integer j,k
  !*************** begin of code ******************************************
  ! periodic boundary conditions in z direction (here assume Ng=2) 
  do j = jmin_tf,jmax_tf
    yu(j,1) = yu(j,Nzt-3)
    yu(j,2) = yu(j,Nzt-2)
    yv(j,1) = yv(j,Nzt-3)
  ...
  !*************** end  uflowsi *******************************************
   ...
   do k = 1,Nzt
     ...
     enddo
   enddo
   ubar = ubar / (dble(jmax_tf-jmin_tf+1)*dble(Nzt))

   scale_flow = umean / ubar
   do k = 1,Nzt 
     do j = jmin_tf,jmax_tf 
      yu(j,k) = yu(j,k) * scale_flow
      yv(j,k) = yv(j,k) * scale_flow
      yw(j,k) = yw(j,k) * scale_flow
     enddo
   enddo
   !***************    scaleu3d  ********************************************
   ...
   do k = 1,Nzt
     ...
     enddo
   enddo
   end subroutine augument
   !************** end of augument *****************************************
end module module_turb_inflow
...
character(len=8) :: expl(0:6) = (/ "wall    ", "periodic", "shear   ", "vel-in  ", "vel-out ", "pressure", "free-out" /)
...
! for walls set the mask to zero  ! 
 if(bdry_cond(1)==0 .or. bdry_cond(1)==2 .or. bdry_cond(1)==3) then
   if(coords(1)==0    ) umask(is-1,js-1:je+1,ks-1:ke+1)=0d0
 endif
 
 if(bdry_cond(4)==0 .or. bdry_cond(4)==2 .or. bdry_cond(4)==3) then
   if(coords(1)==nPx-1) umask(ie,js-1:je+1,ks-1:ke+1)=0d0
 endif

 if(bdry_cond(2)==0 .or. bdry_cond(2)==2 .or. bdry_cond(2)==3) then
   if(coords(2)==0    ) vmask(is-1:ie+1,js-1,ks-1:ke+1)=0d0
 endif 
 
 if(bdry_cond(5)==0 .or. bdry_cond(5)==2 .or. bdry_cond(5)==3) then
   if(coords(2)==nPy-1) vmask(is-1:ie+1,je,ks-1:ke+1)=0d0
 endif 

 if(bdry_cond(3)==0 .or. bdry_cond(3)==2 .or. bdry_cond(3)==3) then
   if(coords(3)==0    ) wmask(is-1:ie+1,js-1:je+1,ks-1)=0d0
 endif
 
 if(bdry_cond(6)==0 .or. bdry_cond(6)==2 .or. bdry_cond(6)==3) then
  ...
  subroutine SetVelocityBC(u,v,w,umask,vmask,wmask,t,dt,AfterProjection)
    ...
    if(bdry_cond(1)==3 .and. coords(1)==0    ) then
       ...
       if ( inject_type == 6 ) then
          ! Interpret ut, vt, wt to stagger grid, namley u, v, w
          do j= js,je
             do k= ks,ke
                u(is-1,j,k)=WallVel(1,1)*ut(j,k) !*umask(is-1,j,k)
                u(is-2,j,k)=WallVel(1,1)*ut(j,k) !*umask(is-2,j,k)
                v(is-1,j,k)= (vt(j,k)+vt(j+1,k))/2 !(vt(j,k)+vt(j+1,k))-v(is,j,k) 
                w(is-1,j,k)= (wt(j,k)+wt(j,k+1))/2 !(wt(j,k)+wt(j,k+1))-w(is,j,k)
             enddo
          enddo
       else
          do j=jmin,jmax
             do k=kmin,kmax
                uinj = uinject(j,k,t)
                u(is-1,j,k)=WallVel(1,1)*uinj
                u(is-2,j,k)=WallVel(1,1)*uinj
                v(is-1,j,k)=0d0
                w(is-1,j,k)=0d0
             enddo
          enddo
...
! ------------------------------------Wall BC with velocity specified----------------------------------------
    ...
    if(bdry_cond(1)==0 .and. coords(1)==0    ) then
        if (d.eq.1) then
            ...
            mom(is-1,:,:)=(2*WallVel(1,d)-u(is,:,:))*(rho2*c(is,:,:) + rho1*(1.d0 - c(is,:,:))) !CHECK!!
        endif
    endif
    ...
    if(bdry_cond(4)==0 .and. coords(1)==nPx-1  ) then
        if (d.eq.1) then
            mom(ie,:,:)=0d0
            mom(ie+1,:,:)=-mom(ie-1,:,:) ! to be corrected, it is not exact
        else !y,z
            mom(ie+1,:,:)=(2*WallVel(2,d)-u(ie,:,:))*(rho2*c(ie,:,:) + rho1*(1.d0 - c(ie,:,:))) !CHECK!!
        endif
    endif
    ...
    if(bdry_cond(2)==0 .and. coords(2)==0    ) then
        if (d.eq.2) then
            ...
            mom(:,js-1,:)=(2*WallVel(3,d)-u(:,js,:))*(rho2*c(:,js,:) + rho1*(1.d0 - c(:,js,:)))
        endif
    endif
    ...
    if(bdry_cond(5)==0 .and. coords(2)==nPy-1) then
        if (d.eq.2) then
            ...
            mom(:,je+1,:)=(2*WallVel(4,d)-u(:,je,:))*(rho2*c(:,je,:) + rho1*(1.d0 - c(:,je,:)))
        endif
    endif
    ...
    if(bdry_cond(3)==0 .and. coords(3)==0    ) then
        if (d.eq.3) then
            ...
            mom(:,:,ks-1)=(2*WallVel(5,d)-u(:,:,ks))*(rho2*c(:,:,ks) + rho1*(1.d0 - c(:,:,ks)))
        endif
    endif
    ...
    if(bdry_cond(6)==0 .and. coords(3)==nPz-1) then
        if (d.eq.3) then
            ...
            mom(:,:,ke+1)=(2*WallVel(6,d)-u(:,:,ke))*(rho2*c(:,:,ke) + rho1*(1.d0 - c(:,:,ke)))
        endif
    endif
...
!  Momentum BCs for shear specified, default is zero shear which equates to impermeable free-slip walls
    if(bdry_cond(1)==2 .and. coords(1)==0    ) then
        if (d.eq.1) then
            mom(is-1,:,:) = 0.d0    
            mom(is-2,:,:) = -mom(is,:,:)    
...
!=================================================================================================
  subroutine do_ghost_vector(us1,us2,us3)
    ...
    function uout(j)
      ...
      if ( inject_type == 3 .or. inject_type == 6) then ! 2d jet
         ...
         call pariserror("uout called with inject_type != 3 or 6")
...
!=================================================================================================
!=================================================================================================
! The Poisson equation for C=color is setup with matrix A as
! A7*Cijk = A1*Ci-1jk + A2*Ci+1jk + A3*Cij-1k + 
!           A4*Cij+1k + A5*Cijk-1 + A6*Cijk+1 + A8
!-------------------------------------------------------------------------------------------------
subroutine SetupDensity(dIdx,dIdy,dIdz,A) 
  use module_grid
  use module_BC
  implicit none
  real(8), dimension(imin:imax,jmin:jmax,kmin:kmax), intent(in) :: dIdx,dIdy,dIdz
...
!   ! Anchor a point to 1 in the other cases

!    if(bdry_cond(1).ne.0.and.bdry_cond(2).ne.0.and.bdry_cond(3).ne.0) then
!       if(coords(1)==0 .and. coords(2)==0 .and. coords(3)==0) then
!           A(is,js,ks,8)=A(is,js,ks,8)+A(is,js,ks,1)
!           A(is,js,ks,1)=0d0
!        endif
!     endif

!   if(check_Poisson_simple(A).ne.0) call pariserror("incorrect setup for density")
contains
  function check_Poisson_simple(A)
    use module_grid
    use module_flow
    implicit none
    include 'mpif.h'
    real(8) :: conda, conda_global
    integer :: ierr
    integer :: check_poisson_simple
    real(8), dimension(is:ie,js:je,ks:ke,8), intent(in) :: A
    
    conda =sum(A(:,:,:,8))
    
    call MPI_ALLREDUCE(conda, conda_global, 1, MPI_DOUBLE_PRECISION, MPI_SUM, MPI_COMM_Domain, ierr)
    if(rank == 0) print *, "sum A8 = ", conda_global
    if (sum(A(:,:,:,8)).lt.1.E-6) check_poisson_simple=0
  end function check_Poisson_simple
...
subroutine Poisson_BCs(A)
  ...
  if(coords(3)==Npz-1) then
     if (bdry_cond(6)==5) then
        ...
        A(:,:,ke,1:6) = 0d0
        A(is,js,ke,2) = 1d0/3d0*P_bc(1); A(is,js,ke,4) = 1d0/3d0*P_bc(3)
        A(ie,js,ke,1) = 1d0/3d0*P_bc(2); A(ie,js,ke,4) = 1d0/3d0*P_bc(3)
        A(is,je,ke,2) = 1d0/3d0*P_bc(1); A(is,je,ke,3) = 1d0/3d0*P_bc(4)
        A(ie,je,ke,1) = 1d0/3d0*P_bc(2); A(ie,je,ke,3) = 1d0/3d0*P_bc(4)
        A(:,:,ke,5) = 1d0/3d0
     endif
  endif
