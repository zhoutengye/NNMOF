...
subroutine get_residual(A,p,norm,error_norm)
  ...
  do k=ks,ke; do j=js,je; do i=is,ie
     ...
     if(norm==1.or.norm==2) then
        ...
        error_norm = max(error_norm,abs(A(i,j,k,8)))
     else
...
!*** Linfinity norm
         ...
         do k=ks,ke; do j=js,je; do i=is,ie
            error_norm=max(error_norm,abs(-p(i,j,k) * A(i,j,k,7) +                       &
                 A(i,j,k,1) * p(i-1,j,k) + A(i,j,k,2) * p(i+1,j,k) +         &
                 A(i,j,k,3) * p(i,j-1,k) + A(i,j,k,4) * p(i,j+1,k) +         &
...
subroutine NewSolverMG(A,p,maxError,beta,maxit,it,ierr,norm,tres2)
  ...
  DO it=1,maxit,1
    ...
    if(norm==1.or.norm==2) then
       ...
       call MPI_ALLREDUCE(resMax, tres2, 1, MPI_DOUBLE_PRECISION, MPI_MAX, MPI_Comm_Cart, ierr) 
    else
       call pariserror("invalid norm")
    endif
