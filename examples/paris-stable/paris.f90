...
! Main program 
!
! Contact: Stephane Zaleski stephane.zaleski@sorbonne-universite.fr
! 
...
Program paris
  ...
  use module_front
  use module_rudman_init ! Subgrid module  
  use module_turb_inflow
  use module_poisson
  use module_mgsolver
!  use module_averages
  use module_IO
...
!---------------------------------------INITIALIZATION--------------------------------------------
  ...
  if(rank==0) then
     if(TwoPhase.or.FreeSurface) then
        if(DoFront2VOF.and..not.DoFront) call pariserror("need DoFront")
        if((.not.DoFront).and.(.not.DoVOF)) call pariserror("need a phase tracking for two-phase")
        if(GetPropertiesFromFront.and.(.not.DoFront)) &
             call pariserror("need Front to get properties from")
        if((.not.GetPropertiesFromFront).and.(.not.DoVOF)) & 
             call pariserror("need VOF to get properties from")
        if(DoMOMCONS.and..not.DoVOF) call pariserror("DoMOMCONS requires DoVOF")
     endif
     if(TwoPhase.and.FreeSurface) call pariserror("cannnot be both TwoPhase and FreeSurface")
  endif

  ! check number of processors
  if ((NumProcess < nPdomain+1).and.DoFront) &
       call pariserror("*** Main: Error with number of processes - Front!")
  ...
  call initialize ! Subgrid
  call check_sanity_in_depth()

  if((DoVOF.or.DoFront2VOF).and.rank<nPdomain) call initialize_VOF
  if(DoLPP) call initialize_LPP

#ifdef PHASE_CHANGE
  do_energy=.true.
  call init_phase_change
  if (rank==0) then
...
!!$  call check_stability
  ...
  if(inject_type==6) then
     call initialize_turb_inflow
  endif ! inject_type 6 for gas inlet turbulence
...
!-------------------------------------------------------------------------------------------------
     ...
     if(ICOut .and. rank<nPdomain) then
        ...
        if((DoVOF.or.DoFront2VOF) .and. .not.restart) then
                 ...
                 if (VTK_OUT(out_fs)) then
                    ...
                    case (1) 
                       ...
                       do k=ks,ke+1; do j=js,je+1; do i=is,ie+1
                          tmp(i,j,k)=ABS((u(i-1,j,k)-u(i,j,k))/dx(i) + &
        ...
        ! stability check positionned after time step change
        call check_stability
     endif ! if ICout
     ...
     if(rank==0) then
        ...
        if (time.eq.0.d0) then
           ...
           write(121,*) '# 1: time 2: shear_y 3: <u> 4: f_x 5: f_y &
                & 6: f_z 7: <rho> 8: <p> 9: <mom>^n &
                & 10: <mom>^(n+1) 11: <C> 12: x_center_of_mass &
                & 13: u^2 C=1  14: u^2 C=0 15: ymom1 or Ek1 16: y_mom2 or Ek2 or vmax 17: turbulence stat 18: <p_infl> &
                & 19: <p_outfl> 20: Area 21: Ep1 22: Ep2 23: Enstrophy1 & 
                & 24: Enstrophy2 25: R2_frac 26: Vof_flag2 27: dpdx & 
                & 28: du/dt 29: time_since_restart'
        else
           open(unit=121,file='stats',position='append')
        endif
        write(121,'(30es14.6e2)') time,stats(1:nstatarray),dpdx,(stats(8)-stats(9))/dt, &
             end_time-start_time
        close(121)
        write(out,'("Step:",I9," Iterations:",I9," cpu(s):",f10.2)')-1,0,end_time-start_time
        write(*,  '("START:", I6," dt=",es16.5e2," time=",es16.5e2," cpu(s):",f11.3   , &
             & " cfl=",es16.5e2)') &
 ...
 !-----------------------------------------MAIN TIME LOOP------------------------------------------
     ...
     do while(time<EndTime .and. itimestep<nstep)
        ...
        if(mod(itimestep,termout)==0) then
           ...
           if(rank==0) &
                write(*,'("Step: ", I6," dt=",es16.5e2," time=",es16.5e2, &
                & " cpu(s):",f11.3," cfl=",es16.5e2)') &
                itimestep,dt,time,end_time-start_time,cflmax
        endif
        ...
        ! generate turbulent fluctuations at the inlet
        if(inject_type==6) then
           ! if experimental data is avaiable
           if(turb_condition==2) read(*,*) data_length
           ! step 1: mean flow (which can change in time)
           call Set_meanflow_turb_inflow(time)
           ! step 2: add turublent fluctuations on the mean flow
           call ImposeTurbFluc
        endif
        ! end turbulence inlet generation
        
        !------------------------------------ADVECTION & DIFFUSION----------------------------------------
        du = 0d0; dv = 0d0; dw = 0d0
        do ii=1, itime_scheme
           if(TwoPhase.and.(.not.GetPropertiesFromFront)) then
             call linfunc(rho,rho1,rho2,DensMean)  ! rho and mu are computed from C^n
             call linfunc(mu,mu1,mu2,ViscMean) ! Note: harmonic mean matches shear stress better
           endif
           call my_timer(2)

#ifdef PHASE_CHANGE
           if ( type<2 .and. .not.(diff_tsat_1D) ) call TempDiffusion(dt) 
           if (type>0 ) then
              call get_heat_source(rho1,rho2,itimestep)
           endif
#endif
           ...
           if(DoVOF) then
#ifdef PHASE_CHANGE
              if (adv_method==0) then
                 call EnergyAdvection
              else
                 if (type==0) then
                    u_l = u
                    v_l = v
                 ...
                 call vofandenergysweeps(itimestep) 
                 ! Advects energy using heat capacity flux from VOF advection  
              endif
#endif
              if (DoMOMCONS) then
                 if (Rudman) then  ! Subgrid
                     call rudman_advect_vof_mom_stg(itimestep,time)  ! Subgrid momcons method of Rudman 
                 else         
                     call vofandmomsweepsstaggered(itimestep,time)
#ifdef PHASE_CHANGE
                     call pariserror('DoMOMCONS not yet compatible with phase change')
#endif
              ...
              call linfunc(rho,rho1,rho2,DensMean)
              call linfunc(mu,mu1,mu2,ViscMean) ! MU was earlier not updated along side rho after the vofsweep
                                                ! Both RHO and MU need to consistent with regard to time-step 
                                                ! in order to compute the coefficients in the diffusion operator 
#ifdef PHASE_CHANGE
              if (type>0) then
                 call sweep_mass_transfer(rho2,dt,dx(is))
                 call ShiftInterfacePhaseChange(dc)
                 ! Need to account for energy change from evap for consistent scheme
                 if (adv_method==1) call dE_PhaseChange(rho1,rho2)
              endif
              !Get topology for ghost cells
              call get_ghost_masks 
              if (TwoPhase.and.(.not.GetPropertiesFromFront)) then
                 if (type<2) call linfunc(kc,kc1,kc2,ArithMean)
                 call linfunc(rho_cp,rho1*cp1,rho2*cp2,ArithMean)
              endif
              if ( (type==2 .or. diff_tsat_1D ) .and. .not.(ngradT_test) ) & 
                   call TempDiffusionImplicit
#endif
              ...
              if (FreeSurface) then
                 ...
                 call my_timer(15)
              endif ! if FreeSurface
           endif ! if doVOF
#ifdef PHASE_CHANGE
           if (type<2 .and. .not.(diff_tsat_1D)) then
              call get_Te_from_energy !energy should now contain Adv and Diff comps
           endif
           call SetTempBC
           call do_all_ghost(Te)
           if (type==2) call get_energy
           ...
           if (.not.FreeSurface) then
              ...
              !---------END FRONT TRACKING send/receive --------------------------------------------
...
!-----------------------------------------PROJECTION STEP-----------------------------------------
           ...
           if (.not.FreeSurface) then
...
#ifdef PHASE_CHANGE
              elseif (do_energy) then
                 call SetupPoisson_phase_change(A)
#endif
              else
              ...
              if (FS_Hypre) then
                 ...
                 call setuppoisson_fs_hypre(u,v,w,umask,vmask,wmask,rho1,dt,A,height,tmp,tag_id)
              else
                 solver_flag = 1
                 call setuppoisson_fs_heights(u,v,w,umask,vmask,wmask,rho1,dt,A,tag_id)
              endif
           endif
           ...
           if(it==maxit) then 
              ...
              residual=res_tab(min(ResNormOrderPressure,3)) !hack to make 100->3
              if (residual/(maxError/MaxDt) > DivergeTol) then 
                 if ( SwitchHYPRESolver .and. HYPRESolverType == 2 ) then
                    ...
                    if (rank == 0) write(*,*) "PFMG failed, switch to SMG...", &
                         itimestep,ii,residual/(maxError/MaxDt),DivergeTol
                    HYPRESolverType = 1
                    if (residual/(maxError/MaxDt) > DivergeTol) p = tmp 
                    !PFMG may diverge,then use old p as IC for iteration
                    call poi_solve(A,p,maxError/MaxDt*ErrorScaleHYPRE,maxit,it,HYPRESolverType)
                    call do_all_ghost(p)
                    call calcResidual(A,p,ResNormOrderPressure,residual)
                    if (residual/(maxError/MaxDt) > DivergeTol ) then  
                       if (rank == 0) write(*,*) "The PFMG/SMG pressure solver diverges !",& 
                           itimestep,ii,residual/(maxError/MaxDt),DivergeTol,HYPRESolverType
                       call pariserror("The PFMG/SMG pressure solver diverges !")
                    end if ! residual
                 else 
                    call pariserror("The pressure solver diverges ! (Adjust DivergeTol&
                         & or Reduce ErrorScaleHYPRE if HYPRE is used!")
                 end if ! SwitchHYPRESolver
              end if !residual
           end if ! END if maximum iteration is reached, check residual
           if(mod(itimestep,termout)==0 .and. ii==1) then
              call calcResiduals(A,p,res_tab)
              residual=res_tab(min(ResNormOrderPressure,3))
              if(rank==0) then
                 write(*,'("  pressure residuals*dt L1:",e8.1,"         L2:",e8.1,"       Linf:",&
                      & e8.1)') res_tab*MaxDt
                 write(*,'("  pressure iterations     :",I8,  " tolerance :",e8.1," norm order:",&
                      & I4)')   it,maxerror,ResNormOrderPressure              
              end if
              ...
              if ( DynamicAdjustPoiTol ) then 
                 if (HYPRE .and. residual/(maxError/MaxDt) > ResRatioDecreaseErrorScaleHYPRE ) then  
...
! END TEMPORARY
                 else if (HYPRE.and.residual/(maxError/MaxDt) < ResRatioIncreaseErrorScaleHYPRE) then 
                    ErrorScaleHYPRE = ErrorScaleHYPRE*2.0d0
                    if (rank == 0) write(*,*) "ErrorScaleHYPRE is increased.",ErrorScaleHYPRE
                 end if ! HYPRE & distinguish residual increase / decrease
           ...
           if (type>0) then
              ...
              ! setup for extension
              call setuppoisson_phase_change_ext(A)
              ! solve Poisson problem
              if(HYPRE_Ps)then
                 call poi_solve(A,p_s,maxError/MaxDt*ErrorScaleHYPRE,maxit,it,HYPRESolverType)
                 call do_all_ghost(p_s)
              else
                 call NewSolver(A,p_s,maxError/MaxDt,beta,maxit,it,ierr,ResNormOrderPressure)
              endif
              if(it>=maxit) then 
                 if (.not.(HYPRE_Ps)) then
                    if(rank==0) write(*,'(" Re-solving Ps with MG ")')
                    p_s=0.0d0
                    call poi_solve(A,p_s,maxError/MaxDt*ErrorScaleHYPRE,maxit,it,HYPRESolverType)
                    call do_all_ghost(p_s)
                 endif
                 call calcResiduals(A,p_s,res_tab)
                 residual=res_tab(min(ResNormOrderPressure,3)) !hack to make 100->3
                 if (residual/(maxError/MaxDt) > DivergeTol) then 
                    call pariserror("Pressure solver diverge in Ps solve")
                 end if !residual
              end if !it
              if(mod(itimestep,termout)==0 .and. ii==1) then
              ...
              ! Project with p_s
              call phase_change_vel_correction
              call SetVelocityBC(u_s,v_s,w_s,umask,vmask,wmask,time,dt,1) 
              ! Will work for wall, to consider rest
              call do_ghost_vector(u_s,v_s,w_s)
           endif
#endif
           ...
           call my_timer(10)
           !-----------------UPDATE COLOR AND/OR DO FRONT2VOF --------------------------------------
           if (DoFront) then
              call SetupDensity(dIdx,dIdy,dIdz,A)
              if(hypre)then
                 call poi_solve(A,color,TolDens,maxit,it,HYPRESolverType)
                 call do_all_ghost(color)
              else
                 call NewSolver(A,color,TolDens,BetaDensity,maxit,it,ierr,ResNormOrderDensity)
              endif
              call my_timer(16)
              call calcResiduals(A,color,res_tab)
!               if(mod(itimestep,nstats).eq.0) then
!                  OPEN(UNIT=89,FILE=TRIM(out_path)//'/color-'//TRIM(int2text(rank,padding))//'.txt')
!                  k=nz/2+2
!                  j=ny/2+2
!                  do i=imin,imax
!                     write(89,'(I2," ",e14.5)') i,color(i,j,k)
!                  enddo
!                  close(unit=89)
!               endif
              if(mod(itimestep,termout)==0) then
                 if(rank==0) then
                    write(*,'("  density residual L1:",e8.1,"         L2:",e8.1,"       Linf:",&
                         & e8.1)') &
                         res_tab
                    write(*,'("  density iterations      :",I8,  " tolerance :",e8.1," norm order:",&
                         & I4)')   &
                         it,TolDens,ResNormOrderDensity
                 endif
              endif
              !adjust color function to 0-1 range
              call check_color()
              ! ... FRONT2VOF  code would go here
              ! ... OUTPUT CALCULATED VOF to compare with color function
              call my_timer(17)
           endif
...
!----------------------------------EXTRAPOLATION FOR FREE SURFACE---------------------------------
           if (DoVOF.and.FreeSurface) then
              ...
              if (mod(itimestep,nstats)==0 .and. mod(ii,itime_scheme)==0) & 
                   call discrete_divergence(u,v,w,itimestep/nstats)
              call my_timer(15) 
           endif !Extrapolation
           !--------------------------------------End free surface stuff --------------------------
...
#ifdef PHASE_CHANGE
           Te = 0.5d0*(Te+Te_old)
           rho_cp = 0.5d0*(rho_cp+rho_cp_old)
           kc = 0.5d0*(kc+kc_old)
           energy = 0.5d0*(energy + energy_old)
#endif
           call my_timer(2)
...
!--------------------------------------------OUTPUT-----------------------------------------------
...
#ifdef PHASE_CHANGE
           if (super_vapour) call NuAvg(time)
#endif
        ...
        end if ! if ( tout ... )  output based on time or timestep
              ...
              if (VTK_OUT(out_fs)) then
                 if (mod(itimestep,NOUT_VTK(out_fs))==0) then
                    ...
                    case (1) 
                       tmp = 0d0
                       do k=ks,ke+1; do j=js,je+1; do i=is,ie+1
                          tmp(i,j,k)=ABS((u(i-1,j,k)-u(i,j,k))/dx(i)+(v(i,j-1,k)-v(i,j,k))/dy(j) &
        ...
        call my_timer(11)
     enddo
     call wrap_up_timer_0()
     !--------------------------------------------End domain-----------------------------------------
     !-----------------------------------------------------------------------------------------------
  else if((rank==nPdomain).and.DoFront) then !front tracking process (rank=nPdomain)
     !-----------------------------------------------------------------------------------------------
     !--------------------------------------------Begin Front----------------------------------------
     call my_timer(8)
     !-----------------------------------------------------------------------------------------------
     ...
     do while(time<EndTime .and. iTimeStep<nstep)
        ...
        do ii=1, itime_scheme
           ...
           call my_timer(5)
           ! Distribute the front among processors. 
...
!--------------------------------------------OUTPUT-----------------------------------------------
     ...
     enddo
!-------------------------------------------------------------------------------------------------
!--------------------------------------------End Front Time Loopp --------------------------------
     call wrap_up_timer_np()
  endif
!------------------------------------------------ End Front --------------------------------------
  ...
  if (.not.FreeSurface) then
     ...
     do k=ks,ke;  do j=js,je; do i=is,ieu    ! CORRECT THE u-velocity 
        if (u_cmask(i,j,k)==0) then
           u(i,j,k)=u(i,j,k)-dt*umask(i,j,k)/rho(i,j,k)* &
     ...
     do k=ks,ke;  do j=js,jev; do i=is,ie    ! CORRECT THE v-velocity
        if (v_cmask(i,j,k)==0) then
           v(i,j,k)=v(i,j,k)-dt*vmask(i,j,k)/rho(i,j,k)* &
     ...
     do k=ks,kew;  do j=js,je; do i=is,ie   ! CORRECT THE w-velocity
        if (w_cmask(i,j,k)==0) then
           w(i,j,k)=w(i,j,k)-dt*wmask(i,j,k)/rho(i,j,k)* &
...
subroutine TimeStepSize(deltaT,vof_phase1)
  ...
  if(rank<nPdomain)then
     h  = minval(dx)
     if (.not.FreeSurface) then
        if ( inject_type == 3 .or. inject_type ==6 ) then 
           vmax_phys = max(1.d-3,max(ugas_inject,uliq_inject))
           vmax      = 0.d0
...
#ifdef PHASE_CHANGE
     if (type<2) then
        dtTdiff = h**2.d0*min(rho1*Cp1/kc1,rho2*Cp2/kc2)/6.d0
...
subroutine calcStats
  ...
  integer :: i0,j0,k0
  real(8) :: stencil3x3(-1:1,-1:1,-1:1),mxyz(3),AREA3D,vmaxproc,vmax

  nstatarray=25
  if(nstatarray > 100) call pariserror("nstatarray too large")
  mystats(1:nstatarray)=0d0;  vmaxproc=0d0; vmax=0d0
  do k=ks,ke;  do j=js,je;  do i=is,ie
     vol = dx(i)*dy(j)*dz(k)
     ! Average u component
     ...
     if(DoVOF) then
        ...
        mystats(12)=mystats(12)+rho(i,j,k)*kenergy*vol*      color(i,j,k)
        mystats(13)=mystats(13)+rho(i,j,k)*kenergy*vol*(1.d0-color(i,j,k))
     end if ! DoVOF
     ! y-momentum 
     if(DoVOF) then
        if (test_turb) then
        ...
        if(test_khi2d) vmaxproc=max(vmaxproc,abs(v(i,j,k))) ! will overwrite stats(15)
     else if (DoFront) then
        mystats(14)=mystats(14)+rho(i,j,k)*0.5d0*(v(i,j,k)+v(i,j+1,k))*vol*(     color(i,j,k))
        mystats(15)=mystats(15)+rho(i,j,k)*0.5d0*(v(i,j,k)+v(i,j+1,k))*vol*(1.d0-color(i,j,k))
     end if ! (DoVOF)
     ! interfacial area
     if (DoVOF ) then
...
! Get pressure at entry and exit
...
#ifdef PHASE_CHANGE
  If (DoVOF) then
     mystats(16)=mystats(16)+rho_cp(i,j,k)*Te(i,j,k)*vol*cvof(i,j,k)
     mystats(17)=mystats(17)+rho_cp(i,j,k)*Te(i,j,k)*vol*(1.0d0-cvof(i,j,k))
  else
     mystats(16)=mystats(16)+rho1*cp1*Te(i,j,k)*vol
  endif
#endif
  ...
  if(test_khi2D) &
       call MPI_ALLREDUCE(vmaxproc,vmax,1, MPI_DOUBLE_PRECISION, MPI_MAX, MPI_COMM_Domain, ierr)
...
!-------------------------------------------------------------------------------------------------
   subroutine calcTurbStats(istep)
      ...
      real(8) :: uc(is:ie,js:je,ks:ke),  vc(is:ie,js:je,ks:ke), & 
                 ...
                 nn1(is:ie,js:je,ks:ke),  nn2(is:ie,js:je,ks:ke), & 
                 nn3(is:ie,js:je,ks:ke) 
      ...
      uc=0.d0;vc=0.d0;wc=0.d0;area=0.d0;nn1=0.d0;nn2=0.d0;nn3=0.d0
      do i= is,ie; do j=js,je; do k=ks,ke
         ...
         if ( cvof(i,j,k) > 0.d0 .and. cvof(i,j,k) < 1.d0 ) then 
            ...
            if ( mxyz(1)/=0.d0 .or. mxyz(2)/=0.d0 .or. mxyz(3)/=0.d0 ) then  
               ...
               nm = sqrt(nm)
               nn1(i,j,k) = abs(mxyz(1)/nm)
               nn2(i,j,k) = abs(mxyz(2)/nm)
               nn3(i,j,k) = abs(mxyz(3)/nm)
            end if ! mxyz
         end if ! cvof
      end do; end do; end do
      
      ! calculate curvature for surface tension term
      ...
      do k = ks,ke
         ...
         turb_vars(is:ie,js:je,64) = turb_vars(is:ie,js:je,64) &
                                   + area(is:ie,js:je,k)*nn1(is:ie,js:je,k)
         turb_vars(is:ie,js:je,65) = turb_vars(is:ie,js:je,65) &
                                   + area(is:ie,js:je,k)*nn2(is:ie,js:je,k)
         turb_vars(is:ie,js:je,66) = turb_vars(is:ie,js:je,66) &
                                   + area(is:ie,js:je,k)*nn3(is:ie,js:je,k)
         turb_vars(is:ie,js:je,67) = turb_vars(is:ie,js:je,67) &
                                   + area(is:ie,js:je,k)**2.d0*nn1(is:ie,js:je,k)*nn1(is:ie,js:je,k)
         turb_vars(is:ie,js:je,68) = turb_vars(is:ie,js:je,68) &
                                   + area(is:ie,js:je,k)**2.d0*nn2(is:ie,js:je,k)*nn2(is:ie,js:je,k)
         turb_vars(is:ie,js:je,69) = turb_vars(is:ie,js:je,69) &
                                   + area(is:ie,js:je,k)**2.d0*nn3(is:ie,js:je,k)*nn3(is:ie,js:je,k)
         turb_vars(is:ie,js:je,70) = turb_vars(is:ie,js:je,70) &
                                   + area(is:ie,js:je,k)**2.d0*nn1(is:ie,js:je,k)*nn2(is:ie,js:je,k)
         turb_vars(is:ie,js:je,71) = turb_vars(is:ie,js:je,71) &
                                   + area(is:ie,js:je,k)**2.d0*nn1(is:ie,js:je,k)*nn3(is:ie,js:je,k)
         turb_vars(is:ie,js:je,72) = turb_vars(is:ie,js:je,72) &
                                   + area(is:ie,js:je,k)**2.d0*nn2(is:ie,js:je,k)*nn3(is:ie,js:je,k)
         end if ! TurbStatsOrder
         ...
         do i=is,ie; do j=js,je
            ...
            turb_vars(i,j,82) = turb_vars(i,j,82) + dudx*cvof(i,j,k)
            turb_vars(i,j,83) = turb_vars(i,j,83) + dvdy*cvof(i,j,k)
            turb_vars(i,j,84) = turb_vars(i,j,84) + dwdz*cvof(i,j,k)
            turb_vars(i,j,85) = turb_vars(i,j,85) + dvdx*cvof(i,j,k)
            turb_vars(i,j,86) = turb_vars(i,j,86) + dwdx*cvof(i,j,k)
            turb_vars(i,j,87) = turb_vars(i,j,87) + dudy*cvof(i,j,k)
            turb_vars(i,j,88) = turb_vars(i,j,88) + dwdy*cvof(i,j,k)
            turb_vars(i,j,89) = turb_vars(i,j,89) + dudz*cvof(i,j,k)
            turb_vars(i,j,90) = turb_vars(i,j,90) + dvdz*cvof(i,j,k)
            turb_vars(i,j,91) = turb_vars(i,j,91) + dudx*dudx*cvof(i,j,k)
            turb_vars(i,j,92) = turb_vars(i,j,92) + dvdy*dvdy*cvof(i,j,k)
            turb_vars(i,j,93) = turb_vars(i,j,93) + dwdz*dwdz*cvof(i,j,k)
            turb_vars(i,j,94) = turb_vars(i,j,94) + dvdx*dvdx*cvof(i,j,k)
            turb_vars(i,j,95) = turb_vars(i,j,95) + dwdx*dwdx*cvof(i,j,k)
            turb_vars(i,j,96) = turb_vars(i,j,96) + dudy*dudy*cvof(i,j,k)
            turb_vars(i,j,97) = turb_vars(i,j,97) + dwdy*dwdy*cvof(i,j,k)
            turb_vars(i,j,98) = turb_vars(i,j,98) + dudz*dudz*cvof(i,j,k)
            turb_vars(i,j,99) = turb_vars(i,j,99) + dvdz*dvdz*cvof(i,j,k)
            turb_vars(i,j,100) = turb_vars(i,j,100) + dvdx*dudy*cvof(i,j,k)
            turb_vars(i,j,101) = turb_vars(i,j,101) + dwdy*dvdz*cvof(i,j,k)
            turb_vars(i,j,102) = turb_vars(i,j,102) + dudz*dwdx*cvof(i,j,k)
            
            turb_vars(i,j,103) = turb_vars(i,j,103) + dudx*uc(i,j,k)
            turb_vars(i,j,104) = turb_vars(i,j,104) + dudy*uc(i,j,k)
            turb_vars(i,j,105) = turb_vars(i,j,105) + dudz*uc(i,j,k)
            turb_vars(i,j,106) = turb_vars(i,j,106) + dvdx*uc(i,j,k)
            turb_vars(i,j,107) = turb_vars(i,j,107) + dwdx*uc(i,j,k)
            turb_vars(i,j,108) = turb_vars(i,j,108) + dvdx*vc(i,j,k)
            turb_vars(i,j,109) = turb_vars(i,j,109) + dvdy*vc(i,j,k)
            turb_vars(i,j,110) = turb_vars(i,j,110) + dvdz*vc(i,j,k)
            turb_vars(i,j,111) = turb_vars(i,j,111) + dudy*vc(i,j,k)
            turb_vars(i,j,112) = turb_vars(i,j,112) + dwdy*vc(i,j,k)
            turb_vars(i,j,113) = turb_vars(i,j,113) + dwdx*wc(i,j,k)
            turb_vars(i,j,114) = turb_vars(i,j,114) + dwdy*wc(i,j,k)
            turb_vars(i,j,115) = turb_vars(i,j,115) + dwdz*wc(i,j,k)
            turb_vars(i,j,116) = turb_vars(i,j,116) + dudz*wc(i,j,k)
            turb_vars(i,j,117) = turb_vars(i,j,117) + dvdz*wc(i,j,k)

            turb_vars(i,j,118) = turb_vars(i,j,118) + dudx*uc(i,j,k)*cvof(i,j,k)
            turb_vars(i,j,119) = turb_vars(i,j,119) + dudy*uc(i,j,k)*cvof(i,j,k)
            turb_vars(i,j,120) = turb_vars(i,j,120) + dudz*uc(i,j,k)*cvof(i,j,k)
            turb_vars(i,j,121) = turb_vars(i,j,121) + dvdx*uc(i,j,k)*cvof(i,j,k)
            turb_vars(i,j,122) = turb_vars(i,j,122) + dwdx*uc(i,j,k)*cvof(i,j,k)
            turb_vars(i,j,123) = turb_vars(i,j,123) + dvdx*vc(i,j,k)*cvof(i,j,k)
            turb_vars(i,j,124) = turb_vars(i,j,124) + dvdy*vc(i,j,k)*cvof(i,j,k)
            turb_vars(i,j,125) = turb_vars(i,j,125) + dvdz*vc(i,j,k)*cvof(i,j,k)
            turb_vars(i,j,126) = turb_vars(i,j,126) + dudy*vc(i,j,k)*cvof(i,j,k)
            turb_vars(i,j,127) = turb_vars(i,j,127) + dwdy*vc(i,j,k)*cvof(i,j,k)
            turb_vars(i,j,128) = turb_vars(i,j,128) + dwdx*wc(i,j,k)*cvof(i,j,k)
            turb_vars(i,j,129) = turb_vars(i,j,129) + dwdy*wc(i,j,k)*cvof(i,j,k)
            turb_vars(i,j,130) = turb_vars(i,j,130) + dwdz*wc(i,j,k)*cvof(i,j,k)
            turb_vars(i,j,131) = turb_vars(i,j,131) + dudz*wc(i,j,k)*cvof(i,j,k)
            turb_vars(i,j,132) = turb_vars(i,j,132) + dvdz*wc(i,j,k)*cvof(i,j,k)

            if ( cvof(i,j,k) > EPSC .and. cvof(i,j,k) < 1.d0-EPSC .and. & 
                  tmp(i,j,k) < 1e6 ) then 
               fsx = -tmp(i,j,k)/dx(i)*sigma*(cvof(i+1,j,k)-cvof(i-1,j,k))/(dx(i)+dx(i+1))
               fsy = -tmp(i,j,k)/dy(j)*sigma*(cvof(i,j+1,k)-cvof(i,j-1,k))/(dy(j)+dy(j+1))
               fsz = -tmp(i,j,k)/dz(k)*sigma*(cvof(i,j,k+1)-cvof(i,j,k-1))/(dz(k)+dz(k+1))
               turb_vars(i,j,133) = turb_vars(i,j,133) + fsx 
               turb_vars(i,j,134) = turb_vars(i,j,134) + fsy 
               turb_vars(i,j,135) = turb_vars(i,j,135) + fsz 
               turb_vars(i,j,136) = turb_vars(i,j,136) + fsx*uc(i,j,k) 
               turb_vars(i,j,137) = turb_vars(i,j,137) + fsy*vc(i,j,k) 
               turb_vars(i,j,138) = turb_vars(i,j,138) + fsz*wc(i,j,k)
            end if ! cvof, tmp, only compute surfacd force on cut cell
         end do; end do !i,j
     ...
     if ( mod(iStep,nStepOutputTurbStats) == 0 ) then 
      ...
      if ( nPdomain > 1 ) then 
         ...
         if ( rank == rootout ) then 
            do i=1,Nx
               do j=1,Ny
                  write(101,'(2(E15.8,1X),138(E25.16,1X))') & 
                  (dble(i)-0.5d0)*dx(is),& 
                  (dble(j)-0.5d0)*dy(js),& 
     ...
     contains 
      subroutine backup_turb_write
         ...
         write(101,*) iSumTurbStats
         do i=is,ie; do j=js,je
            write(101,'(138(E25.16,1X))') turb_vars(i,j,1:num_turb_vars)
         enddo; enddo
         CLOSE(UNIT=101)
...
! calculates the convection terms in momentum equation using the slope limiter
! selected in the argument AdvScheme
...
subroutine momentumConvection_onedim(u,v,w,phi,dphi,d,AdvScheme)
  ...
  integer :: i0,j0,k0
!---- the variable d allows to define the staggered mesh that carries the convected variable ---- 
...
!  use module_solid
  ...
  if (ABS(gx)>1.d-12 .or. ABS(dpdx)>1.d-12) call volumeforce_1D(1,gx,dpdx,fx,du)
  if (ABS(gy)>1.d-12 .or. ABS(dpdy)>1.d-12) call volumeforce_1D(2,gy,dpdy,fy,dv)
  if (ABS(gz)>1.d-12 .or. ABS(dpdz)>1.d-12) call volumeforce_1D(3,gz,dpdz,fz,dw)

contains

  subroutine volumeforce_1D(d,g,dp,f1d,du1d)
    use module_boil
    use module_VOF
    implicit none
    real(8), dimension(imin:imax,jmin:jmax,kmin:kmax), intent(inout) :: du1d, f1d
    real(8), intent(in) :: g, dp
    integer, intent(in) :: d
    integer :: kee, jee, iee
    real(8), dimension(imin:imax,jmin:jmax,kmin:kmax) :: cstag
    real(8), dimension(imin:imax,jmin:jmax,kmin:kmax) :: rhostag

    call get_staggered_fractions (cstag,d)
    rhostag = rho2*cstag + rho1 * (1.0d0- cstag)

    kee=ke; jee=je; iee=ie

    if (d==1) iee = ieu
    if (d==2) jee = jev
    if (d==3) kee = kew

    do k=ks,kee;  do j=js,jee; do i=is,iee
       f1d(i,j,k)=f1d(i,j,k) - dp + g*( rhostag(i,j,k) -&
            ( rho2*ec2*(Te(i,j,k)-T_ref)*cstag(i,j,k) +&
            rho1*ec1*(Te(i,j,k)-T_ref)*(1.0d0-cstag(i,j,k)) ) )
       du1d(i,j,k)=du1d(i,j,k)+f1d(i,j,k)/rhostag(i,j,k)
    enddo; enddo; enddo
  end subroutine volumeforce_1D
...
subroutine initialize
  ...
  use module_IO
  use module_tmpvar
  use module_rudman_init ! Subgrid 
...
! Set the stretched grid
  ...
  if ( xform==0.d0 .and. yform==0.d0 .and. zform==0.d0 ) then 
     dxh(:) = xLength/dfloat(Nx); dyh(:) = yLength/dfloat(Ny); dzh(:) = zLength/dfloat(Nz)
     dx (:) = xLength/dfloat(Nx); dy (:) = yLength/dfloat(Ny); dz (:) = zLength/dfloat(Nz)
  end if ! xform, yform, zform
 
  ! Initialisaton of front density solver
  if(DoFront) toldens=maxerrordensity/(min(minval(dx),minval(dy),minval(dz)))**2

  ! rudman initialization of subgrid co-ordinates
!--------------------------------------------------------------------------------------------------------------
  if(Rudman) then
    cell_size(coarse) = dxh(is)  
    call init_idx()
    call init_rudman_coord(cell_size(fine))
  endif
!--------------------------------------------------------------------------------------------------------------
...
subroutine InitCondition
  ...
  real(8) :: my_ave, residual, res_tab(3)
  real :: erf
  ...
  if(rank<nPdomain)then
     if(restart)then
        if ( DoFront ) then 
           ...
           if (FreeSurface) then
              ...
              if (rank==0) then
                 ...
                 write(*,'("P_ref, R_ref, R_RK, dR_RK, ddR_RK, V_0: ",6e14.5)') &
                            P_ref, R_ref, R_RK, dR_RK, ddR_RK, V_0
              endif
           endif
#ifdef PHASE_CHANGE
           if (.not.GetPropertiesFromFront) then
              call linfunc(kc,kc1,kc2,ArithMean)
              call linfunc(rho_cp,rho1*cp1,rho2*cp2,ArithMean)
           endif
           call SetTempBC
           call do_all_ghost(Te)
        ...
        if(DoVOF) then
           call initconditions_VOF()  ! Subgrid
        ...
        ! Test: inject a drop
         ...
         if ( test_cylinder_advection ) then 
            u = U_init; v = U_init; w = 0d0
         endif
         ! Test: multiphase vortex sheet
        ...
        ! Test: Test Lagrangian particle module
         ...
         if ( test_KHI2D ) call ini_con_KHI2D()
     ...
     if(DoFront) then
        ...
        if(rank==0) then
           write(*,'("  density residual L1:",e8.1,"         L2:",e8.1,"       Linf:",&
                & e8.1)') res_tab
           write(*,'("  density iterations      :",I8,  " tolerance :",e8.1," norm order:",&
                & I4)')  it,TolDens,ResNormOrderDensity
        endif
        call check_color()
     endif
...
contains
  ! -------------------------------------------------------------------
  ! Test: Kelvin-Helmholtz Instability
  ! -------------------------------------------------------------------
  subroutine ini_con_KHI2D()  
    implicit none
    real(8) :: DeltaU=1d0
    real(8) :: deltaKH =  1d-2 !sqrt(2.d0)/16.d0 ! Shear layer thickness
    real(8) :: kwavenumber
    real(8) :: A_u
    real(8) :: yy
    real(8) :: yyh
    ...
    rr=rho1/rho2
    if(rho2<rho1) rr=rho2/rho1

    yy00=0.5d0*yLength 
    kwavenumber=2d0*PI*nwavenumber/xlength
    omega = 0.5d0*DeltaU*kwavenumber
    A_u = omega*IniWaveAmp 
    ! print *, "A_u = ",A_u
    ! w = i omega h = \partial h / \partial t
    ! Correction in vofmodule.f90:
    ! IniWaveAmp = IniWaveAmp_read/(sqrt(rr)*kwavenumber)=  IniWaveAmp_read/omega
    ! A_u = IniWaveAmp_read *and* A_u independent of rr and nwavenumber
    ! print *, "A_u, kwavenumber", A_u, kwavenumber
    w=0; v=0
    !base flow
    do i=imin,imax; do j=jmin,jmax; do k=kmin,kmax
       yy01 = yy00 + IniWaveAmp*cos(kwavenumber*xh(i))
       yy = y(j) - yy01
       if(yy>0) then
          u(i,j,k) = 0.5d0*DeltaU*erf(yy/deltaKH) + A_u*sin(kwavenumber*xh(i))*exp(-kwavenumber*yy)
       else 
          u(i,j,k) = 0.5d0*DeltaU*erf(yy/deltaKH) - A_u*sin(kwavenumber*xh(i))*exp(kwavenumber*yy)
       endif
       yyh = yh(j) - yy01
       if(yyh>0) then
          v(i,j,k) = A_u*cos(kwavenumber*x(i))*exp(-kwavenumber*yyh)
       else if(yyh<0) then
          v(i,j,k) = A_u*cos(kwavenumber*x(i))*exp(kwavenumber*yyh)
       else
          v(i,j,k) = A_u*cos(kwavenumber*x(i))
       endif
    enddo; enddo; enddo
  end subroutine ini_con_KHI2D
...
subroutine ReadParameters
  ...
  namelist /parameters/ out_path,      Nx,            Ny,            Nz,            Ng,          &
                        ...
                        itime_scheme,  nstep,         maxit,        maxErrorDensity,maxError,    &
                        ...
                        ResNormOrderDensity,          BetaDensity,                               &
                        OutVelSpecified,              MaxFluxRatioPresBC,    LateralBdry,        & 
                        HYPRESolverType, SwitchHYPRESolver, DivergeTol,  uinjectPertAmp,         &
                        ResRatioDecreaseErrorScaleHYPRE, ResRatioIncreaseErrorScaleHYPRE,        &
                        ExcludeBoundCellCalcRes, numCellExclude, OrganizeOutFolder,              &  
                        plane, n_p, out_sub, test_MG, MultiGrid, nrelax,u_file, v_file, w_file,  &
                        read_u, read_v, read_w, sym_MG, recordconvergence, DoFront2VOF,          &
                        DoGPU, Rudman,                                                           & ! Subgrid
                        nfx,nfy,nfz,lnx,lny,lnz,ymin_tf,ymax_tf,inlet_turb_intensity,            &
                        turb_condition,data_length
 
  Nx = 0; Ny = 4; Nz = 4 ! this is meant to cause absurd input files that lack the nx value to fail. 
  Ng=2;xLength=1d0;yLength=1d0;zLength=1d0
  gx = 0d0; gy=0d0; gz=0d0; bdry_cond = 0
  dPdx = 0d0;  dPdy = 0d0; dPdz = 0d0
  itime_scheme = 1;  nstep = 0; maxit = 50; maxError = 1d-3; maxErrorDensity = 1d-5  
  beta = 1.2; nout = 1 
  ...
  ResRatioDecreaseErrorScaleHYPRE=2.d0; ResRatioIncreaseErrorScaleHYPRE=0.5d0
  DoFront2VOF = .false.
  do_energy=.false.
  DoGPU= .false.
  Rudman = .false. ! Subgrid
  nfx=8;nfy=8;nfz=8;lnx=4;lny=4;lnz=4;ymin_tf=8.25d-4;ymax_tf=1.6d-3;inlet_turb_intensity=0.00;turb_condition=1;
  data_length = 100
  BetaDensity=1.3
  ...
  if(ResNormOrderPressure/=1.and.ResNormOrderPressure/=2.and.ResNormOrderPressure/=100) &
       call err_no_out_dir("Invalid value of ResNormOrderPressure.")
...
subroutine check_stability() 
  ...
  if(dt*sigma /= 0) then 
     ststab = dx(ng)**3*max(rho1,rho2)/(dt**2*sigma)
  ...
  if(MaxDt*sigma /= 0) then 
     ststab1 = dx(ng)**3*max(rho1,rho2)/(MaxDt**2*sigma)
...
! Check temperature diffusion time scale based on Von Neumann
#ifdef PHASE_CHANGE
  if (type<2) then ! .and. .not.(diff_tsat_1D) ) then
     T_diff = kc1/(rho1*cp1)
     T_diff = MAX(kc1/(rho1*cp1),kc2/(rho2*cp2))
     von_neumann = dx(is)**2.d0/(dt*T_diff)
     if(rank==0) print *, "dx**2/(dt*alpha_T) = ", von_neumann
     if(von_neumann < 6d0) call pariserror("time step too large for Temp diffusion term")
  endif
#endif
...
subroutine check_color()
  ...
  implicit none
  include 'mpif.h'
  real(8) :: cmin, cmax, cmin_global, cmax_global, diff
  integer :: ierr
  cmin = minval(color)
  cmax = maxval(color)

  call MPI_ALLREDUCE(cmin, cmin_global, 1, MPI_DOUBLE_PRECISION, MPI_MIN, MPI_COMM_Domain, ierr)
  call MPI_ALLREDUCE(cmax, cmax_global, 1, MPI_DOUBLE_PRECISION, MPI_MAX, MPI_COMM_Domain, ierr)
  diff = cmax_global - cmin_global 
!   if(rank == 0) then
!      print *, "color global difference = ", diff
!   endif
  color = (color - cmin_global)/diff
!   cmin = minval(color)
!   cmax = maxval(color)
!   call MPI_ALLREDUCE(cmin, cmin_global, 1, MPI_DOUBLE_PRECISION, MPI_MIN, MPI_COMM_Domain, ierr)
!   call MPI_ALLREDUCE(cmax, cmax_global, 1, MPI_DOUBLE_PRECISION, MPI_MAX, MPI_COMM_Domain, ierr)
!   if(rank == 0) then
!      print *, "color global min, max = ", cmin_global, cmax_global
!   endif
  return
