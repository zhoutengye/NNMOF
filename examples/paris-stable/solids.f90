  ...
  SUBROUTINE initialize_solids()
    ...
    if(dosolids) then
       ...
       do i=is,ie; do j=js,je; do k=ks,ke; 
          ...
          else if (solid_type == '2D_nozzle_tb') then
               NozzleThickness = NozzleThick2Cell*dx(is)
               if (( x(i) < NozzleLength) .and.  & 
                   ( (y(j) < radius_liq_inject+NozzleThickness .and. y(j) > radius_liq_inject) & 
                 .or.(y(j) < radius_gas_inject+NozzleThickness .and. y(j) > radius_gas_inject))) then 
                  s1 = 1.d0 
               else
                  s1 =-1.d0
               end if ! x(i),y(j)
