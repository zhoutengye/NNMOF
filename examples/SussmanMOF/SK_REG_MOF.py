...
print(np.shape(f))
print(np.shape(X))
# to change the range of test data, sinply chaenge the subscripts or import other data.
n_test = 10
L = np.zeros((n_test,6))
...
# L2_error_xnorm_lm  = sum(pow(y2_lm[:,0]-Y3[:,0])) / n_test
# L2_error_xnorm_nn  = sum(pow(y2_nn[:,0]-Y3[:,0])) / n_test
# L2_error_xnorm_dt  = sum(pow(y2_dt[:,0]-Y3[:,0])) / n_test
# L2_error_xnorm_knn = sum(pow(y2_knn[:,0]-Y3[:,0]))/ n_test
# L2_error_xnorm_rf  = sum(pow(y2_rf[:,0]-Y3[:,0])) / n_test
# L2_error_ynorm_lm  = sum(pow(y2_lm[:,1]-Y3[:,1])) / n_test
# L2_error_ynorm_nn  = sum(pow(y2_nn[:,1]-Y3[:,1])) / n_test
# L2_error_ynorm_dt  = sum(pow(y2_dt[:,1]-Y3[:,1])) / n_test
# L2_error_ynorm_knn = sum(pow(y2_knn[:,1]-Y3[:,1]))/ n_test
# L2_error_ynorm_rf  = sum(pow(y2_rf[:,1]-Y3[:,1])) / n_test
# L2_error_area_lm  = sum(pow(y2_lm[:,2]-Y3[:,2]))  / n_test
# L2_error_area_nn  = sum(pow(y2_nn[:,2]-Y3[:,2]))  / n_test
# L2_error_area_dt  = sum(pow(y2_dt[:,2]-Y3[:,2]))  / n_test
# L2_error_area_knn = sum(pow(y2_knn[:,2]-Y3[:,2])) / n_test
# L2_error_area_rf  = sum(pow(y2_rf[:,2]-Y3[:,2]))  / n_test
