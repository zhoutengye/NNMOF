...
!    w_{iquad}GQws(jsten,iquad,k)f_{jsten}
      ...
      allocate(ypoints(0:bfact-1),bwG(0:bfact-1))
      ...
      if (gridtype.eq.0) then
       ...
       allocate(ypointsGL(0:bfact),bwGL(0:bfact))
      ...
      do i=0,stenhi(1)
       ...
       do n=1,nvar 
        fxfine(n)=fxfine(n)+fcoarse(D_DECL(i,j,k),n)*wt
       enddo
       wtsum=wtsum+wt
      enddo
      enddo
      ...
      if ((operation_flag.lt.0).or.(operation_flag.gt.9)) then
...
!      call polyinterp_Dmatrix(bfact+1,yLRT,wLRT)
      ...
      if (nmat.ne.num_materials) then
        ...
        write(*,*) nmat, num_materials
       print *,"nmat invalid"
       stop
      endif

      if ((FSI_flag(im).eq.1).or. &
          (FSI_flag(im).eq.2)) then
       is_rigid=1
      ...
      subroutine combine_solid_LS(LS,nmat,solid_dist)
      use probcommon_module
      ...
      enddo ! im=1..nmat
       ...
       ! solid_dist>0 in the solid
      subroutine combine_solid_LS_primary(LS,nmat,solid_dist,im_solid_crit)
      use probcommon_module
      ...
      end subroutine combine_solid_LS_primary
      ...
      enddo ! im=1..nmat
       ...
       ! solid_dist>0 in the solid
       ! solid_dist already initialized
      subroutine combine_solid_LS_INIT(LS,nmat,solid_dist)
      use probcommon_module
      ...
      end subroutine combine_solid_LS_INIT
