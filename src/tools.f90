...
Contains
  Subroutine Visual3DContour(f1, f2, f3, f4, f5, slice_dir, slice_coord)
    ...
    If ( myid .eq. 0 ) Then
      ...
      Write(10,'(a)') "    vis = np.transpose(vis, (1, 2, 0))"
  ...
  Subroutine Visual3DQuiver(u,v,w)
    Implicit None
    ...
    If ( myid .eq. 0 ) Then
      ...
      Write(10,'(a)') "from mayavi import mlab"
      Write(10,'(a)') "f = h5py.File('visual.h5','r')"
      Write(10,'(a)') "u = np.array(f['visual']['u'])"
      Write(10,'(a)') "v = np.array(f['visual']['v'])"
      Write(10,'(a)') "w = np.array(f['visual']['w'])"
      Write(10,'(a)') "u = np.transpose(u, (1, 2, 0))"
      Write(10,'(a)') "v = np.transpose(v, (1, 2, 0))"
      Write(10,'(a)') "w = np.transpose(w, (1, 2, 0))"
      Write(10,'(a)') "mlab.quiver3d(u, v, w)"
      Write(10,'(a)') "nx, ny, nz = u.shape"
      Write(10,'(a)') "f.close()"
      Write(10,'(a)') "mlab.outline(extent=[0,nx,0,ny,0,nz])"
      Write(10,'(a)') "mlab.show()"
      close(10)
      Call system('python vis3dquiver.py')
      ! Call system('rm vis3dquiver.py')
    End If

  End Subroutine Visual3DQuiver
