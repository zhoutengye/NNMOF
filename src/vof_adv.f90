...
!       (1-2) VOF-WY (Eulerian, with divergence correction)
!       (1-3) MOF-CIAM (Lanrangian)
!       (1-4) MOF-WY (Eulerian, no Divergen correction)
!       (1-5) THINC-WY (Eulerian, with Divergen correction)
  ...
  do k=1,nl(3)
    do j=1,nl(2)
      do i=1,nl(1)
        ...
        !     vof2(i,j,k) * c2x(i,j,k) + &
        !     vof1(i+ii,j+jj,k+kk) * c1x(i+ii,j+jj,k+kk) + &
        !     f(i,j,k) * cx(i,j,k) * ( a2 - a1)
        ! my = vof3(i-ii,j-jj,k-kk) * c3y(i-ii,j-jj,k-kk) +&
        !     vof2(i,j,k) * c2y(i,j,k) + &
        !     vof1(i+ii,j+jj,k+kk) * c1y(i+ii,j+jj,k+kk) + &
        !     f(i,j,k) * cy(i,j,k) * (a2 - a1)
        ! mz = vof3(i-ii,j-jj,k-kk) * c3z(i-ii,j-jj,k-kk) +&
        !     vof2(i,j,k) * c2z(i,j,k) + &
        !     vof1(i+ii,j+jj,k+kk) * c1z(i+ii,j+jj,k+kk) + &
        !     f(i,j,k) * cz(i,j,k) * (a2 - a1)
        mx = vof3(i-ii,j-jj,k-kk) * c3x(i-ii,j-jj,k-kk) + &
            vof2(i,j,k) * c2x(i,j,k) + &
            vof1(i+ii,j+jj,k+kk) * c1x(i+ii,j+jj,k+kk)
        my = vof3(i-ii,j-jj,k-kk) * c3y(i-ii,j-jj,k-kk) +&
        ...
        mz = vof3(i-ii,j-jj,k-kk) * c3z(i-ii,j-jj,k-kk) +&
            vof2(i,j,k) * c2z(i,j,k) + &
            vof1(i+ii,j+jj,k+kk) * c1z(i+ii,j+jj,k+kk)
        if ( dir == 1 ) mx = mx + f(i,j,k) * cx(i,j,k) * (a2 - a1)
        if ( dir == 2 ) my = my + f(i,j,k) * cy(i,j,k) * (a2 - a1)
        if ( dir == 3 ) mz = mz + f(i,j,k) * cz(i,j,k) * (a2 - a1)
        f(i,j,k) = vof2(i,j,k) &
            + vof1(i+ii,j+jj,k+kk) &
            + vof3(i-ii,j-jj,k-kk)
        cx(i,j,k) = mx / ( f(i,j,k) + 1e-30 )
